...
function formFromEntry(entry: Partial<DiarioLancamento> | undefined, date: string, celula: number): FormValues {
  return {
...
export default function DiarioLancamentoForm({ aeronaveId, diarioMesId, opcoes, initialData, sugeridaCelula, onSubmit, onCancel, saving }: { aeronaveId: string; diarioMesId: string; opcoes: DiarioOpcoesResponse; initialData?: DiarioLancamento | null; sugeridaCelula: number; onSubmit: (payload: Record<string, unknown>) => Promise<void>; onCancel: () => void; saving: boolean }) {
  const today = new Date().toISOString().slice(0, 10);
  const [form, setForm] = useState<FormValues>(() => formFromEntry(initialData || undefined, today, sugeridaCelula));
  const [error, setError] = useState<string | null>(null);
  useEffect(() => { setForm(formFromEntry(initialData || undefined, today, sugeridaCelula)); setError(null); }, [initialData, diarioMesId, sugeridaCelula]);
