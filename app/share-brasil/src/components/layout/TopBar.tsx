...
import { buscarPerfilColaborador, carregarArquivoColaborador } from "@/lib/colaborador-api";

const AVATAR_PADRAO = "/icon.pilot.png";

export function LogoShare({ compacto = false }: { compacto?: boolean }) {
  return <div className={cn("flex items-center gap-3", compacto && "gap-0")} aria-label="Share Brasil"><div className="relative h-9 w-9 shrink-0"><span className="absolute left-1 top-1 h-[18px] w-[18px] rotate-[-12deg] rounded-[7px] bg-[#4f7d4a]" /><span className="absolute bottom-1 left-1.5 h-[18px] w-[18px] rotate-[18deg] rounded-[7px] bg-[#f1c348]" /><span className="absolute right-1 top-1.5 h-[18px] w-[18px] rotate-[14deg] rounded-[7px] bg-[#167db7]" /></div>{!compacto && <div className="leading-none"><p className="text-[14px] font-extrabold tracking-[.22em] text-foreground">SHARE</p><p className="mt-1 text-[10px] font-semibold italic tracking-[.12em] text-primary">Brasil</p></div>}</div>;
}
