...
function WeatherTophead({ tema }: { tema: Tema }) {
  ...
  return (
    <div className="relative">
      ...
      {open && <div className="absolute right-0 top-[calc(100%+10px)] z-[1000] w-[min(92vw,350px)] rounded-xl border border-border bg-popover p-3 text-popover-foreground shadow-[0_18px_45px_rgba(0,0,0,.3)]">
        <div className="mb-3 flex items-start justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[.14em] text-primary">Condições atuais</p><p className="mt-1 text-xs font-bold">{selectedCity.label}</p></div><span className="rounded-full bg-primary/10 px-2 py-1 text-[9px] font-bold text-primary">AISWEB</span></div>
        ...
        {loading ? <div className="flex items-center gap-2 py-5 text-[10px] text-muted-foreground"><LoaderCircle size={14} className="animate-spin text-primary" /> Consultando METAR...</div> : error ? <div className="mt-3 rounded-lg border border-[#e77b80]/30 bg-[#e77b80]/10 p-3 text-[10px] text-[#ed8c90]">Não foi possível obter o METAR de {icao}. Tente novamente em instantes.</div> : <>
          ...
          <div className="mt-3 flex items-center gap-2 rounded-lg bg-secondary/60 p-2 text-[9px] text-muted-foreground"><Wind size={13} className="shrink-0 text-primary" /><span>METAR <strong className="font-mono text-foreground">{weather?.loc || icao}</strong>{weather?.observed_at ? ` · ${weather.observed_at}` : ""}</span></div>
