...
function CalendarioAgendamento({ mes, semana, dias, visao, confirmados, pendentes, aeronaves, buscaVoo, aoMudarBuscaVoo, aoMudarMes, aoMudarSemana, aoMudarVisao, aoAbrir }: CalendarioAgendamentoProps) {
  const termoVoo = buscaVoo.trim().toLowerCase();
  const eventosPorDia = new Map<string, SolicitacaoVooInterna[]>();
  [...confirmados, ...pendentes]
    .filter((item) => !termoVoo || String(item.numero_voo || "").toLowerCase().includes(termoVoo))
