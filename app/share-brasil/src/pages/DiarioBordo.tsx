...
function LancamentosTableConfigurada({ lancamentos, closed, onEdit, onDelete }: { lancamentos: DiarioLancamento[]; closed: boolean; onEdit: (entry: DiarioLancamento) => void; onDelete: (entry: DiarioLancamento) => void }) {
  ...
  const nomeDestino = (entry: DiarioLancamento) => entry.voo_emprestado ? (entry.socio_tomador_nome || entry.cliente_tomador_nome || entry.socio_nome || entry.cliente_nome || "Não informado") : (entry.socio_nome || entry.cliente_nome || "Não informado");
  const colunas = [
    { key: "ref", label: "COD. REF.", width: 82, className: "text-center text-cyan-300", render: (entry: DiarioLancamento) => `#${entry.numero_sequencial}` },
  ...
  const colunasOcultasNoCelular = new Set(["ac", "dep", "pou", "cor", "tnoit", "ifr", "pousos", "abast", "fuel", "sic"]);
  ...
  return <section className="overflow-hidden rounded-2xl border border-white/[.08] bg-[#101722] shadow-[0_18px_55px_rgba(0,0,0,.14)]">
    ...
    {!exibidos.length ? <EstadoVazio label={aba === "emprestimos" ? "Nenhum voo empréstimo neste mês" : "Nenhum voo registrado neste mês"} /> : aba === "resumo" ? <div className="w-full overflow-hidden"><table className="w-full table-fixed border-collapse text-left text-[10px]"><thead className="border-b border-white/[.08] bg-[#121d2c]"><tr className="text-[11px] font-bold uppercase tracking-[.08em] text-slate-300"><th className="px-4 py-3">CÓD. REF.</th><th className="px-4 py-3">DATA</th><th className="px-4 py-3">TRECHO</th><th className="px-4 py-3">DEP</th><th className="px-4 py-3">POU</th><th className="px-4 py-3 text-blue-300">T VOO</th><th className="px-4 py-3">VOO PARA</th></tr></thead><tbody>{exibidos.map((entry, index) => <tr key={entry.id} className={`border-b border-white/[.06] ${index % 2 === 0 ? "bg-[#101a28]" : "bg-[#0d1521]"} hover:bg-cyan-300/[.06]`}><td className="px-4 py-3"><button type="button" onClick={() => onEdit(entry)} className="font-mono font-bold text-cyan-300 underline decoration-cyan-300/50 underline-offset-2 hover:text-cyan-100">#{entry.numero_sequencial}</button></td><td className="whitespace-nowrap px-4 py-3 text-slate-200">{date(entry.data_registro)}</td><td className="px-4 py-3 font-mono font-bold text-white">{entry.trecho || `${entry.aerodromo_partida} X ${entry.aerodromo_chegada}`}</td><td className="px-4 py-3 font-mono text-slate-300">{time(entry.tempo_dep)}</td><td className="px-4 py-3 font-mono text-slate-300">{time(entry.tempo_pou)}</td><td className="px-4 py-3 font-mono font-bold text-blue-300">{decimal(entry.tempo_voo, 2)}</td><td className="max-w-[260px] truncate px-4 py-3 font-bold text-white">{nomeDestino(entry)}</td></tr>)}</tbody></table></div> : <div className="diario-table-scroll w-full overflow-hidden"><table className="w-full table-fixed border-collapse text-left text-xs" style={{ tableLayout: "fixed" }}><thead className="sticky top-0 z-10 border-b border-white/[.08] bg-[#121d2c]"><tr className="text-[11px] font-bold uppercase tracking-[.08em] text-slate-300">{colunas.map((coluna) => <th key={coluna.key} className={`relative whitespace-nowrap overflow-hidden text-ellipsis border-r border-white/[.08] px-2 py-3 text-[9px] sm:px-3 sm:text-[10px] ${colunasOcultasNoCelular.has(coluna.key) ? "hidden lg:table-cell" : ""} ${coluna.className || ""}`} style={{ width: widths[coluna.key] || undefined }}>{coluna.label}<button type="button" aria-label={`Redimensionar coluna ${coluna.label}`} onPointerDown={(event) => iniciarResize(event, coluna.key, coluna.width)} className="absolute right-0 top-0 h-full w-2 cursor-col-resize touch-none border-0 bg-transparent p-0 hover:bg-cyan-300/30" /></th>)}</tr></thead><tbody className="text-xs text-slate-200">{exibidos.map((entry, index) => <tr key={entry.id} className={`border-b border-white/[.06] ${index % 2 === 0 ? "bg-[#101a28]" : "bg-[#0d1521]"} hover:bg-cyan-300/[.06]`}><td className="relative border-r border-white/[.06] px-3 py-3 text-center font-mono font-bold text-cyan-300"><button type="button" onClick={() => setAcoesAberta(acoesAberta === entry.id ? null : entry.id)} className="underline decoration-cyan-300/50 underline-offset-2 hover:text-cyan-100">#{entry.numero_sequencial}</button>{acoesAberta === entry.id && <div className="absolute left-2 top-10 z-20 flex w-36 flex-col gap-1 rounded-xl border border-white/[.12] bg-[#182333] p-1.5 text-left shadow-xl"><button type="button" onClick={() => { setAcoesAberta(null); onEdit(entry); }} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-white hover:bg-cyan-300/10 hover:text-cyan-300"><Edit3 size={13} /> Editar</button><button type="button" onClick={() => { setAcoesAberta(null); onDelete(entry); }} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-white hover:bg-red-400/10 hover:text-red-300"><Trash2 size={13} /> Excluir</button></div>}</td>{colunas.slice(1).map((coluna) => <td key={coluna.key} className={`whitespace-nowrap overflow-hidden text-ellipsis border-r border-white/[.06] px-2 py-3 font-mono text-[9px] sm:px-3 sm:text-[10px] ${colunasOcultasNoCelular.has(coluna.key) ? "hidden lg:table-cell" : ""} ${coluna.className || ""}`}>{coluna.render(entry)}</td>)}</tr>)}</tbody><tfoot className="hidden border-t-2 border-cyan-300/20 bg-[#182333] text-xs font-bold lg:table-footer-group"><tr><td className="px-3 py-3 text-center text-cyan-300">TOTAL</td><td colSpan={7} /><td className="px-3 py-3 font-mono text-blue-300">{decimal(totais.voo, 2)}</td><td colSpan={3} /><td className="px-3 py-3 text-center text-emerald-300">{totais.pousos}</td><td className="px-3 py-3 text-amber-300">{decimal(totais.abast, 1)}</td><td className="px-3 py-3 text-amber-300">{decimal(totais.fuel, 1)}</td><td colSpan={4} /></tr></tfoot></table></div>}
  </section>;
}
