...
function LancamentosTable({ lancamentos, closed, onNew, onEdit, onDelete }: { lancamentos: DiarioLancamento[]; closed: boolean; onNew: () => void; onEdit: (entry: DiarioLancamento) => void; onDelete: (entry: DiarioLancamento) => void }) {
  ...
  return <section className={`${card} overflow-hidden`}>
    <CabecalhoSecao icon={<BookOpenCheck size={15} />} title="Registros de voo" detail={`${lancamentos.length} voos · clique no número para editar ou excluir um registro`} action={!closed ? <Button type="button" onClick={onNew} className="h-8 gap-1.5 text-[10px]"><Plus size={12} /> Novo voo</Button> : <EtiquetaStatus tone="amber">Mês fechado</EtiquetaStatus>} />
    {lancamentos.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1480px] border-collapse text-left text-[10px]"><thead><tr className="border-b border-border bg-secondary/20 text-[9px] font-bold uppercase tracking-[.08em] text-muted-foreground"><th className={cell}>#</th><th className={cell}>Data</th><th className={cell}>De</th><th className={cell}>Para</th><th className={cell}>AC</th><th className={cell}>DEP</th><th className={cell}>POU</th><th className={cell}>COR</th><th className={`${cell} text-sky-400`}>T VOO</th><th className={`${cell} text-teal-300`}>T DIA</th><th className={`${cell} text-violet-400`}>T NOIT</th><th className={`${cell} text-amber-300`}>IFR</th><th className={`${cell} text-emerald-300`}>Pousos</th><th className={`${cell} text-amber-300`}>Abast+</th><th className={`${cell} text-amber-300`}>Fuel</th><th className={cell}>Célula</th><th className={cell}>PIC</th><th className={cell}>SIC</th><th className={cell}>Voo para</th><th className={cell}>Ações</th></tr></thead><tbody>{lancamentos.map((entry) => <tr key={entry.id} className="border-b border-border/50 last:border-0 hover:bg-primary/[.04]"><td className={`${cell} text-primary`}><button type="button" disabled={closed} onClick={() => onEdit(entry)} className="font-bold underline-offset-2 hover:underline">{entry.numero_sequencial}</button></td><td className={`${cell} font-semibold`}>{date(entry.data_registro)}</td><td className={`${cell} font-mono font-bold text-slate-300`}>{entry.aerodromo_partida}</td><td className={`${cell} font-mono font-bold text-slate-300`}>{entry.aerodromo_chegada}</td><td className={`${cell} text-slate-300`}>{time(entry.tempo_ac)}</td><td className={`${cell} text-slate-300`}>{time(entry.tempo_dep)}</td><td className={`${cell} text-amber-300`}>{time(entry.tempo_pou)}</td><td className={`${cell} text-amber-300`}>{time(entry.tempo_cor)}</td><td className={`${cell} font-bold text-sky-400`}>{hours(entry.tempo_voo)}</td><td className={`${cell} font-bold text-teal-300`}>{hours(entry.tempo_total)}</td><td className={`${cell} font-bold text-violet-400`}>{hours(entry.horas_noturnas)}</td><td className={`${cell} font-bold text-amber-300`}>{hours(entry.tempo_ifr)}</td><td className={`${cell} font-bold text-emerald-300`}>{entry.pousos_total || 0}</td><td className={`${cell} font-bold text-amber-300`}>{decimal(entry.litros_combustivel_abastecido, 0)}</td><td className={`${cell} font-bold text-amber-300`}>{decimal((entry as DiarioLancamento & { consumo_combustivel_total?: number }).consumo_combustivel_total || 0, 0)}</td><td className={`${cell} font-bold text-slate-100`}>{decimal(entry.celula)}h</td><td className={`${cell} max-w-[145px] truncate font-semibold text-slate-200`} title={`${entry.pic_nome_exibicao || entry.pic_nome || entry.pic_canac} · CANAC ${entry.pic_canac}`}>{entry.pic_nome_exibicao || entry.pic_nome || entry.pic_canac || "—"}</td><td className={`${cell} max-w-[145px] truncate text-slate-300`} title={entry.sic_nome_exibicao || entry.sic_nome ? `${entry.sic_nome_exibicao || entry.sic_nome} · CANAC ${entry.sic_canac || "—"}` : entry.sic_canac || "Sem SIC"}>{entry.sic_nome_exibicao || entry.sic_nome || entry.sic_canac || "—"}</td><td className={`${cell} max-w-[170px] truncate font-semibold`} title={entry.socio_nome || entry.cliente_nome || "—"}>{entry.socio_nome || entry.cliente_nome || "—"}{entry.voo_emprestado ? <span className="ml-1 text-[9px] text-amber-300">(emp.)</span> : null}</td><td className={cell}><div className="flex gap-1">{!closed && <><button type="button" onClick={() => onEdit(entry)} title="Editar" className="rounded-md border border-border p-1.5 text-muted-foreground hover:border-primary/40 hover:text-primary"><Edit3 size={12} /></button><button type="button" onClick={() => onDelete(entry)} title="Excluir" className="rounded-md border border-border p-1.5 text-muted-foreground hover:border-red-400/40 hover:text-red-300"><Trash2 size={12} /></button></>}</div></td></tr>)}</tbody></table></div> : <EstadoVazio label="Nenhum voo registrado neste mês" />}
  </section>;
}
