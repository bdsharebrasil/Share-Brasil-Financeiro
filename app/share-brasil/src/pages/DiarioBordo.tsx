...
import { ArrowLeft, BookOpenCheck, ChevronDown, Edit3, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
...
export default function DiarioBordo({ aoVoltar }: { aoVoltar?: () => void }) {
  const now = new Date();
  ...
  if (selected) {
    ...
    return (
      <div className="route-enter space-y-5">
        ...
        {entryEditor !== false && month && !closed && options && <DiarioLancamentoForm aeronaveId={selected.id} diarioMesId={month.id} opcoes={options} initialData={entryEditor || null} sugeridaCelula={Number(month.celula_atual_ttotal || 0)} onSubmit={saveEntry} onCancel={() => setEntryEditor(false)} saving={saving} />}
  ...
  return (
    <div className="route-enter space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {aoVoltar && <Button type="button" variant="outline" onClick={aoVoltar} className="h-9 gap-1.5 text-[10px]"><ArrowLeft size={13} /> Voltar</Button>}
      </div>
      {(error || notice) && <div className={`rounded-lg border p-3 text-[11px] ${error ? "border-red-400/30 bg-red-400/10 text-red-200" : "border-emerald-400/30 bg-emerald-400/10 text-emerald-200"}`}>{error || notice}</div>}
...
function LancamentosTable({ lancamentos, closed, onNew, onEdit, onDelete }: { lancamentos: DiarioLancamento[]; closed: boolean; onNew: () => void; onEdit: (entry: DiarioLancamento) => void; onDelete: (entry: DiarioLancamento) => void }) {
  return <section className={`${card} overflow-hidden`}><CabecalhoSecao icon={<BookOpenCheck size={15} />} title="Registros de voo" detail={`${lancamentos.length} lançamento(s) no mês · tempos em horas decimais`} action={!closed ? <Button type="button" onClick={onNew} className="h-8 gap-1.5 text-[10px]"><Plus size={12} /> Novo voo</Button> : <EtiquetaStatus tone="amber">Mês fechado</EtiquetaStatus>} />{lancamentos.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1220px] text-left text-[10px]"><thead><tr className="border-b border-border text-[9px] font-bold uppercase tracking-[.09em] text-muted-foreground"><th className="px-3 py-3">#</th><th className="px-3 py-3">Data</th><th className="px-3 py-3">Voo / trecho</th><th className="px-3 py-3">AC</th><th className="px-3 py-3">DEP</th><th className="px-3 py-3">POU</th><th className="px-3 py-3">COR</th><th className="px-3 py-3">Tempo</th><th className="px-3 py-3">D/N</th><th className="px-3 py-3">Pousos</th><th className="px-3 py-3">NM</th><th className="px-3 py-3">Célula</th><th className="px-3 py-3">PIC / SIC</th><th className="px-3 py-3">Cotista</th><th className="px-3 py-3">Ações</th></tr></thead><tbody>{lancamentos.map((entry) => <tr key={entry.id} className="border-b border-border/50 last:border-0 hover:bg-secondary/20"><td className="px-3 py-3 font-mono text-primary">{entry.numero_sequencial}</td><td className="px-3 py-3 whitespace-nowrap font-semibold">{date(entry.data_registro)}<span className="mt-1 block text-[9px] text-muted-foreground">{entry.numero_voo || "Sem nº"}</span></td><td className="px-3 py-3"><span className="font-mono font-bold">{entry.aerodromo_partida} → {entry.aerodromo_chegada}</span><span className="mt-1 block max-w-[130px] truncate text-[9px] text-muted-foreground">{entry.trecho || "—"}</span></td><td className="px-3 py-3 font-mono text-primary">{time(entry.tempo_ac)}</td><td className="px-3 py-3 font-mono text-primary">{time(entry.tempo_dep)}</td><td className="px-3 py-3 font-mono text-amber-300">{time(entry.tempo_pou)}</td><td className="px-3 py-3 font-mono text-amber-300">{time(entry.tempo_cor)}</td><td className="px-3 py-3 font-mono font-bold">{decimal(entry.tempo_total)}h</td><td className="px-3 py-3 font-mono"><span className="text-emerald-300">{decimal(entry.horas_diurnas)}</span> / <span className="text-violet-300">{decimal(entry.horas_noturnas)}</span></td><td className="px-3 py-3 font-mono">{entry.pousos_total || 0}</td><td className="px-3 py-3 font-mono">{decimal(entry.distancia_nm)}</td><td className="px-3 py-3 font-mono font-bold">{decimal(entry.celula)}h</td><td className="px-3 py-3"><span className="block font-bold">{entry.pic_nome || entry.pic_canac}</span><span className="mt-1 block text-[9px] text-muted-foreground">{entry.sic_nome || entry.sic_canac || "Sem SIC"}</span></td><td className="px-3 py-3 max-w-[150px] truncate">{entry.socio_nome || entry.cliente_nome || "—"}{entry.voo_emprestado ? <span className="mt-1 block text-[9px] font-bold text-amber-300">Empréstimo</span> : null}</td><td className="px-3 py-3"><div className="flex gap-1">{!closed && <><button type="button" onClick={() => onEdit(entry)} title="Editar" className="rounded-md border border-border p-1.5 text-muted-foreground hover:border-primary/40 hover:text-primary"><Edit3 size={12} /></button><button type="button" onClick={() => onDelete(entry)} title="Excluir" className="rounded-md border border-border p-1.5 text-muted-foreground hover:border-red-400/40 hover:text-red-300"><Trash2 size={12} /></button></>}</div></td></tr>)}</tbody></table></div> : <EstadoVazio label="Nenhum voo registrado neste mês" />}</section>;
}
