...
export default function EmissaoRecibo({ aoVoltar }: { aoVoltar: () => void }) {
  ...
  const visualizarAnexo = async (recibo: ReciboFinanceiro) => {
    if (!recibo.anexo_id) return;
    setErro("");
    setPdfAbrindoId(recibo.id);
    try {
      const blob = await carregarArquivoColaborador(`/api/financeiro/recibos/anexos/${encodeURIComponent(recibo.anexo_id)}/arquivo`);
      const url = URL.createObjectURL(blob);
      setPdfPreviewUrl((atual) => {
        if (atual) URL.revokeObjectURL(atual);
        return url;
      });
      setPdfPreviewNumero(`Anexo · ${recibo.numero_recibo}`);
    } catch (cause) {
      setErro(cause instanceof Error ? `Não foi possível abrir o anexo: ${cause.message}` : "Não foi possível abrir o anexo.");
    } finally {
      setPdfAbrindoId(null);
    }
  };
  ...
  return (
    <div className="route-enter mx-auto max-w-6xl space-y-5">
      ...
      {abaAtiva === "historico" && <HistoricoRecibos recibos={recibos} carregando={carregando} onBuscar={buscarHistorico} onConfirmarReembolso={confirmarReembolso} onCancelar={cancelar} onVisualizarPdf={visualizarPdf} onVisualizarAnexo={visualizarAnexo} pdfAbrindoId={pdfAbrindoId} />}
