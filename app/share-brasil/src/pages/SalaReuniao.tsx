...
export default function SalaReuniao() {
  ...
  const draw = (event: React.PointerEvent<HTMLCanvasElement>) => { if (!isHost) return; const target = event.currentTarget; const rect = target.getBoundingClientRect(); const points = [{ x: (event.clientX - rect.left) / rect.width, y: (event.clientY - rect.top) / rect.height }]; const move = (moveEvent: PointerEvent) => points.push({ x: (moveEvent.clientX - rect.left) / rect.width, y: (moveEvent.clientY - rect.top) / rect.height }); const end = () => { target.releasePointerCapture(event.pointerId); target.removeEventListener("pointermove", move); target.removeEventListener("pointerup", end); if (points.length > 1) { const action = { kind: "path" as const, points, color: "#38bdf8", width: 3 }; drawAction(action); send({ type: "whiteboard", action }); } }; target.setPointerCapture(event.pointerId); target.addEventListener("pointermove", move); target.addEventListener("pointerup", end); };
