...
export default function JornadaVoo({ item, aoFechar }: { item: SolicitacaoVooInterna; aoFechar?: () => void }) {
  ...
  const salvarInicio = async () => {
    ...
    try { await iniciarJornadaVoo(item.id, { data, data_jornada: data, horario_apresentacao: apresentacao, tripulacao_checkin_hora: apresentacao, horario_acionamento: acionamento, tempo_ac: acionamento, horario_ac: acionamento, horario_dep: perna.horario_dep, tempo_dep: perna.horario_dep, origem: perna.origem, aerodromo_partida: perna.origem, destino: perna.destino, aerodromo_chegada: perna.destino, tripulante_id: item.piloto_id }); await carregar(); setMensagem("Check-in, AC e DEP salvos. Volte depois para registrar POU e COR."); }
