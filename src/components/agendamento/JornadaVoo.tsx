...
export default function JornadaVoo({ item, aoFechar }: { item: SolicitacaoVooInterna; aoFechar?: () => void }) {
  ...
  const carregar = async () => {
    ...
    if (atual) {
      ...
      setCorteInicio(hora(atual.horario_corte_inicio));
  ...
  const salvarInicio = async () => {
    ...
    try { await iniciarJornadaVoo(item.id, { data, data_jornada: data, horario_apresentacao: apresentacao, horario_acionamento: acionamento, horario_corte_inicio: corteInicio || undefined, horario_ac: acionamento, horario_dep: perna.horario_dep, origem: perna.origem, aerodromo_partida: perna.origem, destino: perna.destino, aerodromo_chegada: perna.destino, tripulante_id: item.piloto_id }); await carregar(); setMensagem("AC e DEP salvos. Volte depois para registrar o POU e o corte."); }
  ...
  const salvarCorte = async () => {
    ...
    setSalvando(true); setErro(""); setMensagem("");
    try { await atualizarPernaJornada(jornada.id, ultimaPerna.id, { horario_pouso: perna.horario_pouso, horario_corte: perna.horario_corte }); await carregar(); setMensagem("POU e corte salvos. Agora você pode iniciar uma nova perna ou encerrar a jornada."); }
  ...
  return <section className={`${box} space-y-4`}>
    <div className="flex items-start gap-3"><div className="rounded-xl bg-primary/15 p-2 text-primary"><Route size={18} /></div><div className="min-w-0 flex-1"><h2 className="text-sm font-bold">Jornada de voo</h2><p className="text-[10px] text-muted-foreground">Salve a jornada no fluxo operacional: apresentação, AC, DEP e, depois, POU.</p></div>{jornada?.status === "encerrada" && <span className="ml-auto rounded-full bg-secondary px-2 py-1 text-[9px] font-bold text-muted-foreground">Encerrada</span>}<button type="button" onClick={aoFechar} aria-label="Fechar jornada de voo" className="rounded-lg p-1.5 text-muted-foreground hover:bg-secondary hover:text-foreground"><X size={16} /></button></div>
    {!jornada ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Campo label="Apresentação do tripulante *"><input type="time" value={apresentacao} onChange={(e) => setApresentacao(e.target.value)} className={input} /></Campo>
      ...
      <Campo label="Corte do motor inicial"><input type="time" value={corteInicio} onChange={(e) => setCorteInicio(e.target.value)} className={input} /></Campo>
      <div className="sm:col-span-2 lg:col-span-4"><p className="mb-2 text-[10px] text-muted-foreground">A primeira gravação não exige pouso: informe AC e DEP agora e registre POU e corte quando a aeronave pousar.</p><Button onClick={() => void salvarInicio()} disabled={salvando || !data || !perna.origem || !perna.destino || !apresentacao || !acionamento || !perna.horario_dep} className="gap-2 text-xs"><Save size={14} /> Salvar AC e DEP</Button></div>
    </div> : <>
      <div className="grid gap-3 text-[10px] sm:grid-cols-4"><div><span className="text-muted-foreground">Data</span><strong className="block">{dataBr(jornada.data)}</strong></div><div><span className="text-muted-foreground">Apresentação</span><strong className="block">{hora(jornada.horario_apresentacao) || "—"}</strong></div><div><span className="text-muted-foreground">AC</span><strong className="block text-sky-300">{hora(jornada.horario_acionamento) || "—"}</strong></div><div><span className="text-muted-foreground">Ordem registrada</span><strong className="block">AC → DEP → POU</strong></div></div>
