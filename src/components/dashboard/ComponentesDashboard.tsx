...
export function CartaoKpi({ label, value, detail, tone = "blue", icon, trend, className }: { label: string; value: string; detail: string; tone?: TomKpi; icon: ReactNode; trend?: string; className?: string }) {
  ...
  return <div className={cn("group min-w-0 rounded-xl border border-border bg-card/80 p-4 transition-all hover:-translate-y-0.5 hover:border-primary/35", className)}><div className="flex min-w-0 items-start justify-between gap-2"><span className="min-w-0 break-words text-[10px] font-bold uppercase tracking-[.1em] text-muted-foreground">{label}</span><span className={cn("shrink-0 rounded-lg border bg-secondary/70 p-2 shadow-sm", styles[tone])}>{icon}</span></div><div className="mt-4 flex items-end justify-between gap-3"><strong className="font-mono text-[23px] font-medium tracking-[-.05em]">{value}</strong>{trend && <span className={cn("mb-1 flex items-center gap-0.5 text-[9px] font-bold", trend.startsWith("+") ? "text-emerald-600 dark:text-emerald-300" : "text-red-600 dark:text-red-300")}>{trend.startsWith("+") ? <ArrowDownRight size={11} className="rotate-180" /> : <ArrowDownRight size={11} />}{trend}</span>}</div><p className="mt-1 break-words text-[10px] leading-snug text-muted-foreground">{detail}</p></div>;
}
