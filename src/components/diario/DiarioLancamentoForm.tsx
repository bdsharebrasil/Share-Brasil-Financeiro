...
export default function DiarioLancamentoForm({ aeronaveId, diarioMesId, opcoes, initialData, sugeridaCelula, sugestaoTrecho, temDiaria, onSubmit, onCancel, saving }: { aeronaveId: string; diarioMesId: string; opcoes: DiarioOpcoesResponse; initialData?: DiarioLancamento | null; sugeridaCelula: number; sugestaoTrecho?: string; temDiaria?: boolean; onSubmit: (payload: Record<string, unknown>) => Promise<void>; onCancel: () => void; saving: boolean }) {
  ...
  return <section className="relative overflow-hidden rounded-2xl border border-white/[.08] bg-[#101722] shadow-[0_24px_70px_rgba(0,0,0,.22)]"><div className="pointer-events-none absolute -right-24 -top-28 h-64 w-64 rounded-full bg-cyan-400/[.07] blur-3xl" /><div className="relative flex items-center justify-between gap-3 border-b border-white/[.08] bg-[#0d1521] px-5 py-4"><div className="flex items-center gap-2.5"><span className="rounded-xl bg-cyan-400/10 p-2 text-cyan-300"><BookOpen size={16} /></span><div><h2 className="text-sm font-bold text-white">{initialData ? "Editar lançamento de voo" : "Novo lançamento de voo"}</h2><p className="mt-0.5 text-xs text-slate-400">Campos do diário digital · dados gravados no D1</p></div></div><button type="button" onClick={onCancel} className="rounded-xl border border-white/[.08] p-2 text-slate-400 transition-colors hover:border-white/[.2] hover:text-white"><X size={15} /></button></div><div className="relative space-y-6 p-5">
    ...
    <div className="sticky bottom-0 -mx-5 -mb-5 flex flex-wrap items-center justify-end gap-2 border-t border-white/[.08] bg-[#0d1521]/95 px-5 py-4 backdrop-blur"><Button type="button" variant="outline" onClick={onCancel} className="h-9 border-white/[.12] bg-white/[.03] text-xs text-slate-300 hover:bg-white/[.08] hover:text-white">Cancelar</Button><Button type="button" disabled={saving} onClick={() => void submit()} className="h-9 gap-2 rounded-lg bg-cyan-400 px-4 text-xs font-extrabold text-slate-950 hover:bg-cyan-300"><Save size={13} />{saving ? "Salvando..." : initialData ? "Atualizar lançamento" : "Registrar lançamento"}</Button></div>
  </div></section>;
}

function FormSection({ icon, title, children }: { icon: ReactNode; title: string; children: ReactNode }) { return <div className="rounded-2xl border border-white/[.07] bg-[#0d1521]/60 p-4"><div className="mb-3 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.14em] text-cyan-300"><span>{icon}</span>{title}</div>{children}</div>; }
