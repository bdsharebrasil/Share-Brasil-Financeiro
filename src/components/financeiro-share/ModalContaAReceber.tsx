...
export function ModalContaAReceber({ conta, aberto, onFechar, onConfirmar }: ModalContaAReceberProps) {
  ...
  async function confirmar() {
    ...
    for (const pagamento of pagamentos) {
      ...
      if (!pagamento.conta_bancaria_id.trim() || valorCentavos <= 0 || !pagamento.data_pagamento || !distribuicao.length) {
        setErro('Cada pagamento precisa de data, conta bancária, valor positivo e ao menos um rateio.'); return;
      }
      ...
      if (distribuicao.reduce((sum, r) => sum + r.valor_centavos, 0) !== valorCentavos) {
  ...
  return <Dialog open={aberto} onOpenChange={(v) => !v && onFechar()}>
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
      ...
      <div className="space-y-4 py-2">
        <div className="grid grid-cols-3 gap-3 rounded-md border bg-muted/30 p-3 text-sm"><div><span className="text-muted-foreground">Total</span><strong className="block">R$ {money(esperado)}</strong></div><div><span className="text-muted-foreground">Já recebido</span><strong className="block">R$ {money(recebido)}</strong></div><div><span className="text-muted-foreground">Saldo</span><strong className="block">R$ {money(saldo)}</strong></div></div>
        {pagamentos.map((p, index) => <section key={p.id} className="space-y-3 rounded-md border p-3">
          ...
          <div className="grid gap-3 sm:grid-cols-3"><div className="space-y-1"><Label>Pagador real</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={p.tipo_pagador} onChange={(e) => atualizar(p.id, { tipo_pagador: e.target.value as TipoPagador })}><option value="COTISTA">Cotista</option><option value="SHARE">Share</option><option value="HOLDING">Holding</option></select></div><div className="space-y-1"><Label>ID do pagador</Label><Input value={p.pagadorId} disabled={p.tipo_pagador === 'SHARE'} onChange={(e) => atualizar(p.id, { pagadorId: e.target.value })} placeholder={p.tipo_pagador === 'SHARE' ? 'Não aplicável' : 'ID real'} /></div><div className="space-y-1"><Label>Valor pago (R$)</Label><Input value={p.valor} onChange={(e) => atualizar(p.id, { valor: e.target.value })} /></div></div>
          <div className="grid gap-3 sm:grid-cols-3"><div className="space-y-1"><Label>Data</Label><Input type="date" value={p.data_pagamento} onChange={(e) => atualizar(p.id, { data_pagamento: e.target.value })} /></div><div className="space-y-1"><Label>Conta bancária</Label><Input value={p.conta_bancaria_id} onChange={(e) => atualizar(p.id, { conta_bancaria_id: e.target.value })} placeholder="ID da conta" /></div><div className="space-y-1"><Label>Forma</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={p.forma_pagamento} onChange={(e) => atualizar(p.id, { forma_pagamento: e.target.value })}><option>PIX</option><option>Transferência</option><option>Boleto</option><option>Dinheiro</option><option>Cartão</option></select></div></div>
          <div className="space-y-2"><Label>Distribuição entre rateios</Label>{rateios.length ? rateios.map((r) => <div key={r.id} className="grid grid-cols-[1fr_140px] items-center gap-2 text-sm"><span>Rateio {r.id.slice(0, 8)} · esperado R$ {money(Number(r.valor_rateado_centavos))} · pago R$ {money(Number(r.valor_pago_real_centavos || 0))}</span><Input value={p.rateios[r.id] ?? '0,00'} onChange={(e) => atualizarRateio(p.id, r.id, e.target.value)} aria-label={`Valor no rateio ${r.id}`} /></div>) : <p className="text-sm text-muted-foreground">A conta não possui rateios disponíveis.</p>}</div>
