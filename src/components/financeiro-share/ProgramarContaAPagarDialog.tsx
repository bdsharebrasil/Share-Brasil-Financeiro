...
import { buscarCotistasAeronave, enviarAnexoRecibo, programarContaAPagarRecibo, type CategoriaClienteRecibo, type CotistaAeronave, type Recibo } from "@/lib/colaborador-api";
...
export default function ProgramarContaAPagarDialog({ open, recibo, aeronaves, categorias, onOpenChange, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    setAeronaveId(recibo.aeronave_id || ""); setDataVencimento(recibo.data_vencimento || hoje()); setCategoriaId(""); setSubcategoria(""); setTipoRateio("FIXO"); setPeriodicidade("ÚNICO"); setObservacoes(""); setBoleto(null); setErro(""); setCotistas([]); setPercentuais({});
  }, [open, recibo]);
  ...
  const salvar = async () => {
    ...
    try {
      const boletoSalvo = boleto ? await enviarAnexoRecibo(boleto, recibo.id) : null;
      await programarContaAPagarRecibo(recibo.id, { aeronave_id: aeronaveId, data_vencimento: dataVencimento, categoria_id: categoriaId || null, categoria_nome: categoriaSelecionada?.nome || null, subcategoria_1: subcategoria || null, subcategoria_2: null, subcategoria_3: null, subcategoria_4: null, tipo_rateio: tipoRateio, periodicidade, observacoes: observacoes.trim() || null, boleto_url: boletoSalvo?.url || null, linhas: selecionados.map((cotista) => ({ cotista_id: cotista.id, percentual_uso: Number(percentuais[cotista.id]) })) });
