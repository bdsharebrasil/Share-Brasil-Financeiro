...
import { Bell, ChevronDown, CloudRain, CloudSun, Clock3, Globe2, LoaderCircle, LogOut, Menu, UserRound, Wind } from "lucide-react";
...
export function BarraSuperior({ ambiente, podeAcessarGestor, tema, aoTrocarAmbiente, aoAlternarTema, aoAbrirMenu, aoAbrirPerfil, aoAbrirMensagens, aoSair }: { ambiente: Ambiente; podeAcessarGestor: boolean; tema: Tema; aoTrocarAmbiente: (ambiente: Ambiente) => void; aoAlternarTema: () => void; aoAbrirMenu: () => void; aoAbrirPerfil: () => void; aoAbrirMensagens: () => void; aoSair: () => void | Promise<void> }) {
  const [horario, setHorario] = useState(new Date());
  const [avatar, setAvatar] = useState(AVATAR_PADRAO);
  const [mensagensNaoLidas, setMensagensNaoLidas] = useState(0);
  const [perfilAberto, setPerfilAberto] = useState(false);
  const [pontoAtivo, setPontoAtivo] = useState(() => window.localStorage.getItem(PONTO_ATIVO_STORAGE) === "1");
  ...
  useEffect(() => {
    const atualizarPonto = (event: Event) => {
      const ativo = (event as CustomEvent<{ ativo?: boolean }>).detail?.ativo;
      if (typeof ativo === "boolean") setPontoAtivo(ativo);
    };
    const sincronizarPonto = (event: StorageEvent) => {
      if (event.key === PONTO_ATIVO_STORAGE) setPontoAtivo(event.newValue === "1");
    };
    window.addEventListener(PONTO_ATIVO_EVENTO, atualizarPonto);
    window.addEventListener("storage", sincronizarPonto);
    return () => {
      window.removeEventListener(PONTO_ATIVO_EVENTO, atualizarPonto);
      window.removeEventListener("storage", sincronizarPonto);
    };
  }, []);

  useEffect(() => {
    let ativo = true;
    const atualizar = () => void buscarContagemMensagensNaoLidas().then(({ unread }) => {
      if (ativo) setMensagensNaoLidas(Number(unread || 0));
    }).catch(() => {
  ...
  return <header className="safe-area-header sticky top-0 z-30 relative flex min-h-[var(--app-header-height)] items-center justify-between gap-2 border-b border-border/70 bg-background/90 px-3 backdrop-blur-xl md:grid md:grid-cols-[1fr_auto_1fr] md:px-7"><div className="flex min-w-0 flex-1 items-center gap-3"><button type="button" onClick={aoAbrirMenu} aria-label="Abrir menu" className="rounded-lg p-2 text-muted-foreground hover:bg-secondary hover:text-foreground md:hidden"><Menu size={20} /></button><div className="md:hidden"><LogoShare compacto /></div><label className="sr-only" htmlFor="dashboard-mobile">Selecionar dashboard</label><select id="dashboard-mobile" value={ambiente} onChange={(event) => aoTrocarAmbiente(event.target.value as Ambiente)} className="h-9 max-w-[128px] rounded-lg border border-border bg-card px-2 text-[10px] font-bold text-foreground outline-none focus:border-primary md:hidden" style={tema === "light" ? { backgroundColor: "rgb(255, 255, 255)" } : undefined}>{(["operacoes", "financeiro", "share-brasil", ...(podeAcessarGestor ? ["gestor" as Ambiente] : [])] as Ambiente[]).map((item) => <option key={item} value={item}>{nomesAmbiente[item]}</option>)}</select></div><div className="hidden min-w-0 shrink-0 items-center justify-self-center gap-1 rounded-xl border border-border bg-card/70 px-1.5 py-1 md:flex md:-translate-x-10" style={tema === "light" ? { backgroundColor: "rgb(255, 255, 255)" } : undefined}>{(["operacoes", "financeiro", "share-brasil", ...(podeAcessarGestor ? ["gestor" as Ambiente] : [])] as Ambiente[]).map((item) => <button type="button" key={item} onClick={() => aoTrocarAmbiente(item)} className={cn("rounded-lg px-3 py-2 text-[11px] font-bold transition-all", ambiente === item ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>{nomesAmbiente[item]}</button>)}</div><div className="flex flex-1 items-center justify-end gap-2"><div className={cn("hidden items-center gap-2 rounded-lg border border-border/70 bg-card/65 px-3 py-2 font-mono text-[10px] text-muted-foreground transition-colors lg:flex", pontoAtivo && ambiente === "share-brasil" && "border-emerald-400/40 bg-emerald-400/10 text-emerald-200")} title="Horário local do dispositivo" style={tema === "light" && !(pontoAtivo && ambiente === "share-brasil") ? { backgroundColor: "rgb(255, 255, 255)" } : undefined}><Clock3 size={13} className={cn(pontoAtivo && ambiente === "share-brasil" ? "text-emerald-300" : "text-primary")} /><span className="text-foreground">{horaLocal}</span><span className={cn("text-primary/70", pontoAtivo && ambiente === "share-brasil" && "text-emerald-300")}>{pontoAtivo && ambiente === "share-brasil" ? "Ponto ativo" : "Local"}</span></div><div className="hidden items-center gap-2 rounded-lg border border-border/70 bg-card/65 px-3 py-2 font-mono text-[10px] text-muted-foreground xl:flex" title="Horário Universal Coordenado" style={tema === "light" ? { backgroundColor: "rgb(255, 255, 255)" } : undefined}><Globe2 size={13} className="text-primary" /><span className="text-foreground">{horaUtc}</span><span className="text-primary/70">UTC</span></div><WeatherTophead tema={tema} /><button type="button" onClick={aoAbrirMensagens} aria-label={mensagensNaoLidas > 0 ? `${mensagensNaoLidas} mensagens não lidas` : "Abrir mensagens"} title={mensagensNaoLidas > 0 ? `${mensagensNaoLidas} mensagens não lidas` : "Abrir mensagens"} className="relative rounded-lg border border-transparent p-2 text-muted-foreground transition-colors hover:border-border hover:bg-secondary hover:text-foreground"><Bell size={17} />{mensagensNaoLidas > 0 && <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-[#f1c348] px-1 text-center text-[9px] font-bold leading-4 text-[#17120a]">{mensagensNaoLidas > 99 ? "99+" : mensagensNaoLidas}</span>}</button><div className="relative ml-1"><button type="button" onClick={() => setPerfilAberto((aberto) => !aberto)} aria-label="Abrir meu perfil" aria-expanded={perfilAberto} className="h-9 w-9 overflow-hidden rounded-lg bg-[#0b4a78] ring-2 ring-primary/10"><img src={avatar} alt="Avatar do perfil" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.src = AVATAR_PADRAO; }} /></button>{perfilAberto && <div className="absolute right-0 top-[calc(100%+10px)] z-[1000] w-48 rounded-xl border border-border bg-popover p-1.5 text-popover-foreground shadow-[0_18px_45px_rgba(0,0,0,.35)]"><button type="button" onClick={() => { setPerfilAberto(false); aoAbrirPerfil(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-xs font-semibold transition-colors hover:bg-secondary"><UserRound size={15} className="text-primary" /> Meu perfil</button><button type="button" onClick={() => { setPerfilAberto(false); void aoSair(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-xs font-semibold text-[#d96b72] transition-colors hover:bg-[#e77b80]/10"><LogOut size={15} /> Sair da conta</button></div>}</div></div></header>;
}
