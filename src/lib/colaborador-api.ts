...
export function enviarDocumentoGestao(colaboradorId: string, arquivo: File, categoria: string) { const body = new FormData(); body.append("arquivo", arquivo, arquivo.name); body.append("categoria", categoria); return colaboradorRequest<DocumentoGestao>(`/api/gestor/gestao-colaborador/${encodeURIComponent(colaboradorId)}/documentos`, { method: "POST", body }); }
