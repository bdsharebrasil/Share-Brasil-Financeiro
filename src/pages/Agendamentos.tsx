...
export default function Agendamentos() {
  ...
  const agendamentos = useMemo(() => {
    ...
    return (painel?.agendamentos || []).filter((item) => {
...
function Cronograma({ solicitacoes, nomesTripulantes, busca, aoMudarBusca, aoAbrir, aoChecklist, aoJornada, aoExcluir }: { solicitacoes: SolicitacaoVooInterna[]; nomesTripulantes: Map<string, string>; busca: string; aoMudarBusca: (busca: string) => void; aoAbrir: (item: SolicitacaoVooInterna) => void; aoChecklist: (item: SolicitacaoVooInterna) => void; aoJornada: (item: SolicitacaoVooInterna) => void; aoExcluir: (item: SolicitacaoVooInterna) => void }) {
  return <section className="overflow-hidden rounded-xl border border-border bg-card/75"><CabecalhoSecao icon={<ClipboardList size={15} />} title="Cronograma de Voos" detail="Todas as solicitações de voo" action={<div className="relative"><Search size={13} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input value={busca} onChange={(event) => aoMudarBusca(event.target.value)} placeholder="Buscar cliente, rota ou voo" className="h-8 w-full pl-8 text-[10px] sm:w-56" /></div>} />{solicitacoes.length ? <><div className="grid gap-3 p-3 sm:grid-cols-2 xl:grid-cols-3">{solicitacoes.map((item) => <CartaoCronograma key={item.id} item={item} nomesTripulantes={nomesTripulantes} aoAbrir={aoAbrir} aoChecklist={aoChecklist} aoJornada={aoJornada} aoExcluir={aoExcluir} />)}</div><div className="hidden"><table className="w-full min-w-[980px] text-left"><thead><tr className="border-b border-border text-[9px] font-bold uppercase tracking-[.11em] text-muted-foreground"><th className="px-4 py-3">Número do voo</th><th className="px-4 py-3">Cliente / sócio</th><th className="px-4 py-3">Rota</th><th className="px-4 py-3">Período / horários</th><th className="px-4 py-3">Aeronave</th><th className="px-4 py-3">Tripulação</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Ação</th></tr></thead><tbody>{solicitacoes.map((item) => <tr key={item.id} className="border-b border-border/60 last:border-0 hover:bg-secondary/20"><td className="px-4 py-3 font-mono text-[10px] font-bold text-primary">{item.numero_voo || "A gerar na confirmação"}</td><td className="px-4 py-3"><p className="max-w-[180px] truncate text-[10px] font-bold">{item.voo_emprestado !== "nao" && item.socio_emprestimo_nome ? item.socio_emprestimo_nome : item.socio_nome || item.cliente_razao_social || "Titular não informado"}</p><p className="mt-1 text-[9px] text-muted-foreground">{item.codigo_cliente || "Código pela cotista"}</p></td><td className="px-4 py-3 font-mono text-[10px] text-muted-foreground">{item.origem} → {item.destino}</td><td className="px-4 py-3"><p className="text-[10px] font-semibold">{formatarData(item.data_agendada)}{item.data_fim && item.data_fim !== item.data_agendada ? ` até ${formatarData(item.data_fim)}` : ""}</p><p className="mt-1 text-[9px] text-muted-foreground">UTC: {formatarHorarioAgendamento(item.horario_previsto_agendamento).utc}</p><p className="text-[9px] text-muted-foreground">Brasília: {formatarHorarioAgendamento(item.horario_previsto_agendamento).brasilia}</p></td><td className="px-4 py-3 font-mono text-[10px]">{item.matricula_registro || "A definir"}</td><td className="px-4 py-3 text-[10px] text-muted-foreground">{item.piloto_id ? "Escalada" : "Pendente"}</td><td className="px-4 py-3"><EtiquetaStatus tone={tomStatus(item.status)}>{statusLabel(item.status)}</EtiquetaStatus></td><td className="px-4 py-3 text-right"><div className="flex items-center justify-end gap-3"><button type="button" onClick={() => aoAbrir(item)} className="text-[10px] font-bold text-primary hover:underline">Detalhes</button>{item.status === "aprovada" && <><PesoBalanceamentoFicha item={item} /><button type="button" onClick={() => aoChecklist(item)} className="text-[10px] font-bold text-emerald-400 hover:underline">{item.checklist_status === "concluido" ? "Checklist concluído - ver" : "Iniciar checklist"}</button>{item.checklist_status === "concluido" && <button type="button" onClick={() => aoJornada(item)} className="text-[10px] font-bold text-sky-400 hover:underline">Iniciar jornada de voo</button>}</>}{item.status === "em_voo" && <button type="button" onClick={() => aoJornada(item)} className="text-[10px] font-bold text-sky-300 hover:underline">Continuar jornada em voo</button>}<button type="button" onClick={() => aoExcluir(item)} className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-[#e77b80]/10 hover:text-[#ed8c90]" title="Excluir agendamento" aria-label={`Excluir agendamento ${item.numero_voo || item.id}`}><Trash2 size={14} /></button></div></td></tr>)}</tbody></table></div><div className="grid gap-3 p-3 md:hidden">{solicitacoes.map((item) => <article key={item.id} className="rounded-xl border border-border/70 bg-background/25 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-mono text-[10px] font-bold text-primary">{item.numero_voo || "A gerar na confirmação"}</p><p className="mt-1 truncate text-[11px] font-bold">{item.voo_emprestado !== "nao" && item.socio_emprestimo_nome ? item.socio_emprestimo_nome : item.socio_nome || item.cliente_razao_social || "Titular não informado"}</p><p className="mt-1 text-[10px] text-muted-foreground">{item.origem} → {item.destino}</p></div><EtiquetaStatus tone={tomStatus(item.status)}>{statusLabel(item.status)}</EtiquetaStatus></div><div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><Detalhe label="Período" valor={`${formatarData(item.data_agendada)}${item.data_fim && item.data_fim !== item.data_agendada ? ` até ${formatarData(item.data_fim)}` : ""}`} /><Detalhe label="Aeronave" valor={item.matricula_registro || "A definir"} /><Detalhe label="Horário UTC" valor={formatarHorarioAgendamento(item.horario_previsto_agendamento).utc} /><Detalhe label="Tripulação" valor={item.piloto_id ? "Escalada" : "Pendente"} /></div><div className="mt-3 flex flex-wrap gap-2 border-t border-border/60 pt-3"><Button type="button" variant="outline" onClick={() => aoAbrir(item)} className="h-8 flex-1 text-[10px]">Detalhes</Button>{item.status === "aprovada" && <><PesoBalanceamentoFicha item={item} /><button type="button" onClick={() => aoChecklist(item)} className="h-8 rounded-md border border-emerald-400/30 px-2.5 text-[10px] font-bold text-emerald-400">{item.checklist_status === "concluido" ? "Checklist concluído" : "Iniciar checklist"}</button>{item.checklist_status === "concluido" && <button type="button" onClick={() => aoJornada(item)} className="h-8 rounded-md border border-sky-400/30 px-2.5 text-[10px] font-bold text-sky-400">Jornada</button>}</>}{item.status === "em_voo" && <button type="button" onClick={() => aoJornada(item)} className="h-8 rounded-md border border-sky-400/30 px-2.5 text-[10px] font-bold text-sky-300">Continuar jornada</button>}<button type="button" onClick={() => aoExcluir(item)} className="h-8 rounded-md border border-border p-1.5 text-muted-foreground hover:text-[#ed8c90]" title="Excluir agendamento" aria-label={`Excluir agendamento ${item.numero_voo || item.id}`}><Trash2 size={14} /></button></div></article>)}</div></> : <EstadoVazio label="Nenhuma solicitação encontrada" />}</section>;
}

function CartaoCronograma({ item, nomesTripulantes, aoAbrir, aoChecklist, aoJornada, aoExcluir }: { item: SolicitacaoVooInterna; nomesTripulantes: Map<string, string>; aoAbrir: (item: SolicitacaoVooInterna) => void; aoChecklist: (item: SolicitacaoVooInterna) => void; aoJornada: (item: SolicitacaoVooInterna) => void; aoExcluir: (item: SolicitacaoVooInterna) => void }) {
