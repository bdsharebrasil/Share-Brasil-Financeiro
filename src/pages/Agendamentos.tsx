...
function formatarDuracaoJornada(jornada: import("@/lib/colaborador-api").JornadaVoo) {
  const minutos = (jornada.pernas || []).reduce((total, perna) => {
    if (!perna.horario_ac || !perna.horario_corte) return total;
    const ac = new Date(perna.horario_ac).getTime();
    const corte = new Date(perna.horario_corte).getTime();
    return total + (Number.isFinite(ac) && Number.isFinite(corte) && corte >= ac ? Math.round((corte - ac) / 60000) : 0);
  }, 0);
