...
import { Calendar as CalendarIcon, CalendarDays, Check, ChevronLeft, ChevronRight, ClipboardList, Clock3, LayoutGrid, List, Plane, Plus, RefreshCw, Search, Trash2, Users, X } from "lucide-react";
...
import JornadaVoo from "@/components/agendamento/JornadaVoo";
import PesoBalanceamentoFicha from "@/components/agendamento/PesoBalanceamentoFicha";
...
function normalizarAgendamentos(itens: SolicitacaoVooInterna[] | null | undefined) {
  const porId = new Map<string, SolicitacaoVooInterna>();
  for (const item of Array.isArray(itens) ? itens : []) {
    if (!item?.id) continue;
    const existente = porId.get(item.id);
    if (!existente || String(item.atualizado_em || item.criado_em) >= String(existente.atualizado_em || existente.criado_em)) porId.set(item.id, item);
  }
  return [...porId.values()];
}
...
export default function Agendamentos() {
  ...
  const [novaDisponibilidade, setNovaDisponibilidade] = useState({ tripulante_id: "", data_inicio: dataIso(new Date()), data_fim: dataIso(new Date()), status: "aviso" as "aviso" | "ferias" | "folga" | "atestado_medico" | "treinamento" | "acompanhando_manutencao" | "disponivel", observacoes: "" });
  const carregamentoAtual = useRef(0);

  const carregar = useCallback(async (silencioso = false) => {
    const idCarregamento = ++carregamentoAtual.current;
    ...
    try {
      ...
      if (idCarregamento !== carregamentoAtual.current) return;
      setPainel({
        ...painelAtual,
        agendamentos: normalizarAgendamentos(painelAtual?.agendamentos),
        aeronaves: Array.isArray(painelAtual?.aeronaves) ? painelAtual.aeronaves : [],
        tripulacao: Array.isArray(painelAtual?.tripulacao) ? painelAtual.tripulacao : [],
  ...
  const agendamentos = useMemo(() => normalizarAgendamentos(painel?.agendamentos), [painel?.agendamentos]);
  const confirmados = useMemo(() => agendamentos.filter((item) => ["aprovada", "em_voo"].includes(item.status)), [agendamentos]);
  const encerrados = useMemo(() => agendamentos.filter((item) => item.status === "encerrada"), [agendamentos]);
  ...
  const abrirSolicitacao = (item: SolicitacaoVooInterna) => {
    ...
    setMotivo("");
    setHistoricoJornadas([]);
    setCarregandoHistorico(true);
    void buscarJornadaVoo(item.id).then((jornada) => {
      setHistoricoJornadas(jornada?.historico || (jornada ? [jornada] : []));
    }).catch(() => setHistoricoJornadas([])).finally(() => setCarregandoHistorico(false));
  };
  const navegarParaDataCalendario = (data: string) => {
    const dataVoo = new Date(`${data}T00:00:00`);
    if (Number.isNaN(dataVoo.getTime())) return;
    setMes(primeiroDiaMes(dataVoo));
    setSemanaCalendario(inicioSemana(dataVoo));
  };
  const aprovar = async () => {
  ...
  const finalizar = async (item: SolicitacaoVooInterna) => {
    if (!window.confirm(`Finalizar o agendamento ${item.numero_voo || 'sem número'}? Ele sairá do cronograma, mas o número do voo será preservado no histórico.`)) return;
    setProcessando(true);
    try {
      await finalizarAgendamento(item.id);
      if (selecionada?.id === item.id) setSelecionada(null);
      toast({ title: 'Agendamento finalizado', description: 'A operação foi concluída e saiu do cronograma.' });
      await carregar(true);
    } catch (error) { toast({ title: 'Não foi possível finalizar', description: error instanceof Error ? error.message : 'Encerre todas as jornadas antes de finalizar.', variant: 'destructive' }); }
    finally { setProcessando(false); }
  };
  ...
  return <div className="route-enter min-w-0 space-y-5">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><IndicadorPagina>Operações / agendamento de voo</IndicadorPagina><h1 className="text-2xl font-extrabold tracking-[-.04em] md:text-[30px]">Agendamento de Voo</h1><p className="mt-1.5 max-w-2xl text-xs leading-relaxed text-muted-foreground">Organize solicitações, aeronaves, confirmação de voos e escala da tripulação em uma única central.</p></div><div className="flex w-full flex-wrap gap-2 md:w-auto"><Button type="button" onClick={() => setMostrarNovo((atual) => !atual)} className="flex-1 gap-2 bg-primary text-primary-foreground sm:flex-none"><Plus size={14} /> Novo agendamento</Button><Button type="button" variant="outline" onClick={() => void carregar(true)} disabled={atualizando} className="flex-1 gap-2 border-border bg-card text-xs sm:flex-none"><RefreshCw size={14} className={atualizando ? "animate-spin" : ""} /> Atualizar</Button></div></div>
    ...
    {selecionada && ["aprovada", "em_voo", "encerrada"].includes(selecionada.status) && jornadaAberta && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-3 backdrop-blur-sm sm:p-6"><div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-2xl"><JornadaVoo item={selecionada} aoFechar={() => { setJornadaAberta(false); setSelecionada(null); void carregar(true); }} /></div></div>}
  </div>;
}
...
type CalendarioAgendamentoProps = {
  ...
  buscaVoo: string;
  aoMudarBuscaVoo: (busca: string) => void;
  aoNavegarParaData: (data: string) => void;
  aoAbrir: (item: SolicitacaoVooInterna) => void;
};

function CalendarioAgendamento({ mes, semana, dias, visao, confirmados, pendentes, aeronaves, buscaVoo, aoMudarBuscaVoo, aoNavegarParaData, aoMudarMes, aoMudarSemana, aoMudarVisao, aoAbrir }: CalendarioAgendamentoProps) {
  const termoVoo = buscaVoo.trim().toLowerCase();
  const todosEventos = useMemo(() => [...confirmados, ...pendentes], [confirmados, pendentes]);
  const resultadosBusca = useMemo(() => {
    if (!termoVoo) return [];
    return todosEventos.filter((item) => [item.numero_voo, item.codigo_cliente, item.cliente_razao_social, item.socio_nome, item.origem, item.destino].filter(Boolean).join(" ").toLowerCase().includes(termoVoo)).slice(0, 8);
  }, [todosEventos, termoVoo]);
  const resultadoPrincipal = resultadosBusca[0];
  useEffect(() => {
    if (termoVoo.length >= 2 && resultadoPrincipal?.data_agendada) aoNavegarParaData(resultadoPrincipal.data_agendada);
  }, [termoVoo, resultadoPrincipal?.data_agendada]);
  const eventosPorDia = new Map<string, SolicitacaoVooInterna[]>();
  todosEventos
    .filter((item) => !termoVoo || [item.numero_voo, item.codigo_cliente, item.cliente_razao_social, item.socio_nome, item.origem, item.destino].filter(Boolean).join(" ").toLowerCase().includes(termoVoo))
    .forEach((item) => eventosPorDia.set(item.data_agendada, [...(eventosPorDia.get(item.data_agendada) || []), item]));
  ...
  return <div className="space-y-5"><section className="overflow-hidden rounded-xl border border-border bg-card/75"><CabecalhoSecao icon={<CalendarDays size={15} />} title="Calendário de agendamento" detail={descricao} action={<div className="flex flex-wrap items-center justify-end gap-2"><div className="relative"><Search size={12} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Buscar voo agendado por número" value={buscaVoo} onChange={(event) => aoMudarBuscaVoo(event.target.value)} placeholder="Buscar nº do voo..." className="h-8 w-full pl-8 text-[10px] sm:w-44" /></div><div className="flex rounded-md border border-border p-0.5"><button type="button" onClick={() => aoMudarVisao("semanal")} className={`rounded px-2 py-1 text-[9px] font-bold ${visao === "semanal" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}>Semanal</button><button type="button" onClick={() => aoMudarVisao("mensal")} className={`rounded px-2 py-1 text-[9px] font-bold ${visao === "mensal" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}>Mensal</button></div><div className="flex items-center gap-1"><button type="button" onClick={() => mudarPeriodo(-1)} aria-label={visao === "semanal" ? "Semana anterior" : "Mês anterior"} className="rounded-md p-1.5 text-muted-foreground hover:bg-secondary"><ChevronLeft size={15} /></button><span className="min-w-[110px] text-center text-[10px] font-bold sm:min-w-[150px]">{periodo}</span><button type="button" onClick={() => mudarPeriodo(1)} aria-label={visao === "semanal" ? "Próxima semana" : "Próximo mês"} className="rounded-md p-1.5 text-muted-foreground hover:bg-secondary"><ChevronRight size={15} /></button></div></div>} />{termoVoo && <div className="border-b border-border bg-secondary/10 px-3 py-2"><p className="mb-2 text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Resultados relacionados</p>{resultadosBusca.length ? <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-4">{resultadosBusca.map((item) => <button key={item.id} type="button" onClick={() => { aoNavegarParaData(item.data_agendada); aoAbrir(item); }} className="rounded-md border border-border/70 bg-card/60 p-2 text-left transition-colors hover:border-primary/50 hover:bg-secondary/40"><p className="truncate text-[10px] font-bold text-primary">{item.numero_voo || "Sem número de voo"}</p><p className="truncate text-[9px] text-foreground">{item.codigo_cliente || item.cliente_razao_social || item.socio_nome || "Cliente não informado"}</p><p className="truncate text-[9px] text-muted-foreground">{item.origem} → {item.destino} · {formatarData(item.data_agendada)}</p></button>)}</div> : <p className="text-[10px] text-muted-foreground">Nenhum voo encontrado para “{buscaVoo}”.</p>}</div>}<div className="grid grid-cols-7 border-b border-border bg-secondary/20">{nomesDias.map((dia) => <div key={dia} className="px-2 py-2 text-center text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">{dia}</div>)}</div><div className="grid grid-cols-7">{diasVisao.map((dia) => { const chave = dataIso(dia); const eventos = eventosPorDia.get(chave) || []; const noMes = visao === "semanal" || dia.getMonth() === mes.getMonth(); const hoje = chave === dataIso(new Date()); return <div key={chave} className={`min-h-[150px] border-b border-r border-border/60 p-2 ${noMes ? "bg-card/25" : "bg-background/25"}`}><div className={`mb-2 flex h-6 w-6 items-center justify-center rounded-full font-mono text-[10px] ${hoje ? "bg-primary text-primary-foreground" : noMes ? "text-foreground" : "text-muted-foreground"}`}>{dia.getDate()}</div><div className="space-y-1.5">{eventos.map((item) => <ResumoVooCalendario key={item.id} item={item} aeronave={item.matricula_registro || aeronaves.find((aeronave) => aeronave.id === item.aeronave_id)?.matricula_registro || "Aeronave a definir"} aoAbrir={aoAbrir} />)}</div></div>; })}</div></section></div>;
}
...
function Escala({ tripulacao, escala, disponibilidades, visao, aoMudarVisao, aoMarcar }: { tripulacao: TripulanteAgendamento[]; escala: EscalaAgendamento[]; disponibilidades: DisponibilidadeTripulacao[]; visao: VisaoEscala; aoMudarVisao: (visao: VisaoEscala) => void; aoMarcar: () => void }) {
  ...
  const statusTone = (status?: string) => status === "atestado_medico" ? "border-red-300/20 bg-red-300/10 text-red-200" : status === "treinamento" ? "border-violet-300/20 bg-violet-300/10 text-violet-200" : status === "acompanhando_manutencao" ? "border-sky-300/20 bg-sky-300/10 text-sky-200" : status === "folga" ? "border-slate-300/20 bg-slate-300/10 text-slate-200" : status === "atestado_medico" ? "border-red-300/20 bg-red-300/10 text-red-200" : status === "treinamento" ? "border-violet-300/20 bg-violet-300/10 text-violet-200" : status === "acompanhando_manutencao" ? "border-sky-300/20 bg-sky-300/10 text-sky-200" : status === "folga" ? "border-slate-300/20 bg-slate-300/10 text-slate-200" : status === "ferias" ? "border-rose-300/20 bg-rose-300/10 text-rose-200" : status === "aviso" ? "border-amber-300/20 bg-amber-300/10 text-amber-200" : status === "disponivel" ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-200" : "border-white/[.08] bg-white/[.04] text-slate-500";
...
function CartaoCronograma({ item, nomesTripulantes, aoAbrir, aoChecklist, aoJornada, aoFinalizar, aoExcluir }: { item: SolicitacaoVooInterna; nomesTripulantes: Map<string, string>; aoAbrir: (item: SolicitacaoVooInterna) => void; aoChecklist: (item: SolicitacaoVooInterna) => void; aoJornada: (item: SolicitacaoVooInterna) => void; aoFinalizar: (item: SolicitacaoVooInterna) => void; aoExcluir: (item: SolicitacaoVooInterna) => void }) {
  const cliente = item.voo_emprestado !== "nao" && item.socio_emprestimo_nome ? item.socio_emprestimo_nome : item.socio_nome || item.cliente_razao_social || item.codigo_cliente || "Cliente não informado";
  const podeAbrirJornada = ["aprovada", "em_voo", "encerrada"].includes(item.status);
  return <article className="rounded-xl border border-border/70 bg-background/30 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-mono text-[10px] font-bold text-primary">{item.numero_voo || "A gerar na confirmação"}</p><p className="mt-1 truncate text-[10px] font-semibold">{cliente}</p></div><EtiquetaStatus tone={tomStatus(item.status)}>{statusLabel(item.status)}</EtiquetaStatus></div><div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 text-[10px]"><div><span className="block text-[9px] text-muted-foreground">Rota</span><strong className="font-mono">{item.origem} → {item.destino}</strong></div><div><span className="block text-[9px] text-muted-foreground">Período</span><strong>{formatarData(item.data_agendada)}{item.data_fim && item.data_fim !== item.data_agendada ? ` a ${formatarData(item.data_fim)}` : ""}</strong></div><div><span className="block text-[9px] text-muted-foreground">Aeronave</span><strong className="font-mono">{item.matricula_registro || "A definir"}</strong></div><div><span className="block text-[9px] text-muted-foreground">Tripulação</span><strong>{item.piloto_id ? [item.piloto_nome || (item.piloto_id ? nomesTripulantes.get(item.piloto_id) : null), item.copiloto_nome || (item.copiloto_id ? nomesTripulantes.get(item.copiloto_id) : null)].filter(Boolean).join(" / ") || "Tripulante não localizado" : "A definir"}</strong></div></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => aoAbrir(item)} className="min-h-9 flex-1 rounded-lg border border-primary/30 px-3 py-2 text-[10px] font-bold text-primary hover:bg-primary/10">Abrir</button>{item.status === "aprovada" && <button type="button" onClick={() => aoChecklist(item)} className="min-h-9 flex-1 rounded-lg border border-emerald-400/30 px-3 py-2 text-[10px] font-bold text-emerald-300 hover:bg-emerald-400/10">Checklist</button>}{podeAbrirJornada && <button type="button" onClick={() => aoJornada(item)} className="min-h-9 flex-1 rounded-lg border border-border px-3 py-2 text-[10px] font-bold text-foreground hover:bg-secondary">Jornada</button>}{Number(item.jornadas_encerradas || 0) > 0 && Number(item.jornadas_abertas || 0) === 0 && <button type="button" onClick={() => aoFinalizar(item)} className="min-h-9 flex-1 rounded-lg border border-amber-400/30 px-3 py-2 text-[10px] font-bold text-amber-300 hover:bg-amber-400/10">Operação concluída</button>}<button type="button" onClick={() => aoExcluir(item)} className="min-h-9 rounded-lg border border-red-400/30 px-3 py-2 text-[10px] font-bold text-red-300 hover:bg-red-400/10">Excluir</button></div></article>;
}

function DetalhesSolicitacao({ selecionada, tripulacao, pilotoId, copilotoId, motivo, processando, historicoJornadas, carregandoHistorico, aoFechar, aoPiloto, aoCopiloto, aoMotivo, aoAprovar, aoReprovar }: { selecionada: SolicitacaoVooInterna; tripulacao: TripulanteAgendamento[]; pilotoId: string; copilotoId: string; motivo: string; processando: boolean; historicoJornadas: import("@/lib/colaborador-api").JornadaVoo[]; carregandoHistorico: boolean; aoFechar: () => void; aoPiloto: (valor: string) => void; aoCopiloto: (valor: string) => void; aoMotivo: (valor: string) => void; aoAprovar: () => void; aoReprovar: () => void }) { return <section className="overflow-visible rounded-xl border border-primary/25 bg-card"><CabecalhoSecao icon={<ClipboardList size={15} />} title="Detalhes da solicitação" detail={`${selecionada.origem} → ${selecionada.destino} · ${formatarData(selecionada.data_agendada)}`} action={<button type="button" onClick={aoFechar} aria-label="Fechar detalhes" className="rounded-md p-1 text-muted-foreground hover:bg-secondary"><X size={16} /></button>} /><div className="grid gap-5 p-5 lg:grid-cols-[1fr_.9fr]"><div className="grid grid-cols-2 gap-3 text-[10px] sm:grid-cols-4"><Detalhe label="Cliente / sócio" valor={selecionada.socio_nome || selecionada.cliente_razao_social || selecionada.codigo_cliente || "Não informado"} /><Detalhe label="Aeronave" valor={`${selecionada.matricula_registro || "A definir"} · ${selecionada.modelo || "—"}`} /><Detalhe label="Horário" valor={(() => { const horario = formatarHorarioAgendamento(selecionada.horario_previsto_agendamento); return `UTC: ${horario.utc} · Brasília: ${horario.brasilia}`; })()} /><Detalhe label="Período" valor={`${formatarData(selecionada.data_agendada)}${selecionada.data_fim && selecionada.data_fim !== selecionada.data_agendada ? ` até ${formatarData(selecionada.data_fim)}` : ""}`} /><Detalhe label="Passageiros" valor={String(selecionada.numero_passageiros || 1)} /><Detalhe label="Número do voo" valor={selecionada.numero_voo || "Será gerado na confirmação"} /><Detalhe label="Status" valor={statusLabel(selecionada.status)} /><Detalhe label="Horário UTC" valor={selecionada.horario_previsto_agendamento || "A definir"} /><Detalhe label="Empréstimo" valor={selecionada.voo_emprestado === "sim" ? `Sim · ${selecionada.cliente_emprestimo_nome || selecionada.socio_emprestimo_nome || "Cedente não informado"}` : "Não"} /></div><HistoricoJornadas jornadas={historicoJornadas} carregando={carregandoHistorico} />{selecionada.status === "pendente" ? <div className="rounded-xl border border-border bg-secondary/20 p-4"><p className="text-[11px] font-bold">Confirmar solicitação</p><p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">A aprovação gera o número com o código de cotista e confirma os tripulantes selecionados.</p><div className="mt-4 space-y-3"><div className="space-y-1.5"><Label className="text-[10px]">PIC (piloto em comando)</Label><SearchableCombobox items={tripulacao.map((item) => ({ id: item.id, label: `${item.nome_completo} · CANAC ${item.canac} · ${item.origem === "freelancer" ? "Freelancer" : "Tripulação"}` }))} value={pilotoId} onChange={(valor) => aoPiloto(valor)} placeholder="Selecione o comandante" searchPlaceholder="Digite o nome ou CANAC" emptyMessage="Nenhum tripulante encontrado." icon={<Users size={14} />} /></div><div className="space-y-1.5"><Label className="text-[10px]">SIC (segundo em comando) <span className="text-muted-foreground">(opcional)</span></Label><SearchableCombobox items={tripulacao.filter((item) => item.id !== pilotoId).map((item) => ({ id: item.id, label: `${item.nome_completo} · CANAC ${item.canac} · ${item.origem === "freelancer" ? "Freelancer" : "Tripulação"}` }))} value={copilotoId} onChange={(valor) => aoCopiloto(valor)} placeholder="Sem copiloto" searchPlaceholder="Digite o nome ou CANAC" emptyMessage="Nenhum tripulante encontrado." icon={<Users size={14} />} /></div><div className="space-y-1.5"><Label htmlFor="motivo-reprovacao" className="text-[10px]">Motivo da reprovação</Label><Textarea id="motivo-reprovacao" value={motivo} onChange={(event) => aoMotivo(event.target.value)} placeholder="Preencha apenas se for reprovar" className="min-h-16 text-[11px]" /></div><div className="flex flex-wrap gap-2 pt-1"><Button type="button" onClick={aoAprovar} disabled={processando} className="gap-2 bg-[#2bbf8a] text-[#03150e] hover:bg-[#45d5a5]"><Check size={14} /> Aprovar e gerar voo</Button><Button type="button" onClick={aoReprovar} disabled={processando} variant="outline" className="gap-2 border-[#e77b80]/40 bg-transparent text-[#ed8c90] hover:bg-[#e77b80]/10"><X size={14} /> Reprovar</Button></div></div></div> : <div className="rounded-xl border border-border bg-secondary/20 p-4"><p className="text-[11px] font-bold">{selecionada.status === "encerrada" ? "Voo encerrado" : "Solicitação aprovada"}</p><p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">Status atual: <strong className="text-foreground">{statusLabel(selecionada.status)}</strong>{selecionada.motivo_rejeicao ? ` · ${selecionada.motivo_rejeicao}` : ""}.</p>{selecionada.status === "aprovada" && <div className="mt-4 grid gap-3"><div className="rounded-lg border border-primary/20 bg-primary/5 p-3"><p className="text-[9px] font-bold uppercase tracking-wide text-primary">Mensagem para o cliente</p><textarea readOnly value={`Olá ${selecionada.socio_nome || selecionada.cliente_razao_social || "Cotista"}, seu voo ${selecionada.numero_voo || ""} foi confirmado para o dia ${formatarData(selecionada.data_agendada)} com o tripulante ${tripulacao.find((tripulante) => tripulante.id === selecionada.piloto_id)?.nome_completo || "escalado"}.`} className="mt-2 min-h-20 w-full resize-none rounded-md border border-border bg-background p-2 text-[10px] leading-relaxed" /></div><div className="rounded-lg border border-sky-300/20 bg-sky-300/5 p-3"><p className="text-[9px] font-bold uppercase tracking-wide text-sky-300">Mensagem para o tripulante</p><textarea readOnly value={`Olá ${tripulacao.find((tripulante) => tripulante.id === selecionada.piloto_id)?.nome_completo || "tripulante escalado"}, você foi escalado para o voo - ${selecionada.numero_voo || ""}, no dia ${formatarData(selecionada.data_agendada)} na aeronave ${selecionada.matricula_registro || "a definir"} com o cotista ${selecionada.socio_nome || selecionada.cliente_razao_social || "não informado"} às ${formatarHorarioAgendamento(selecionada.horario_previsto_agendamento).utc} UTC.`} className="mt-2 min-h-20 w-full resize-none rounded-md border border-border bg-background p-2 text-[10px] leading-relaxed" /></div></div>}</div>}</div></section>; }
