...
function formatarDuracaoJornada(jornada: import("@/lib/colaborador-api").JornadaVoo) {
  if (jornada.minutos_jornada !== null && jornada.minutos_jornada !== undefined) {
    const minutosRegistrados = Math.max(0, Number(jornada.minutos_jornada));
    return `${Math.floor(minutosRegistrados / 60)}h${String(minutosRegistrados % 60).padStart(2, "0")}`;
  }
  const inicio = jornada.horario_apresentacao || jornada.horario_acionamento;
  const fim = jornada.horario_corte_final;
...
function HistoricoJornadas({ jornadas, carregando }: { jornadas: import("@/lib/colaborador-api").JornadaVoo[]; carregando: boolean }) {
  return <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3"><p className="text-[9px] font-bold uppercase tracking-wide text-primary">Histórico operacional</p>{carregando ? <p className="mt-2 text-[10px] text-muted-foreground">Carregando jornadas...</p> : jornadas.length ? <div className="mt-2 space-y-2">{[...jornadas].sort((a, b) => Number(a.numero_jornada) - Number(b.numero_jornada)).map((jornada) => <div key={jornada.id} className="rounded-lg border border-border/70 bg-background/40 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-[10px]">Jornada {jornada.numero_jornada} · dia {dataJornada(jornada.data)}</strong><span className="rounded-full bg-secondary px-2 py-1 text-[9px] font-bold">{jornada.status === "encerrada" ? "Encerrada" : "Em andamento"}</span></div><div className="mt-2 grid grid-cols-2 gap-2 text-[10px] sm:grid-cols-5"><Detalhe label="Check-in" valor={horaJornada(jornada.horario_apresentacao)} /><Detalhe label="AC" valor={horaJornada(jornada.horario_acionamento)} /><Detalhe label="Corte início" valor={horaJornada(jornada.horario_corte_inicio)} /><Detalhe label="Corte final" valor={horaJornada(jornada.horario_corte_final)} /><Detalhe label="Tempo total" valor={formatarDuracaoJornada(jornada)} /></div>{jornada.pernas?.length ? <p className="mt-2 text-[9px] text-muted-foreground">{jornada.pernas.map((perna) => `${perna.origem} → ${perna.destino} · DEP ${horaJornada(perna.horario_dep)} · POU ${horaJornada(perna.horario_pouso)} · COR ${horaJornada(perna.horario_corte)}`).join(" | ")}</p> : null}</div>)}</div> : <p className="mt-2 text-[10px] text-muted-foreground">Nenhuma jornada iniciada.</p>}</div>;
}
