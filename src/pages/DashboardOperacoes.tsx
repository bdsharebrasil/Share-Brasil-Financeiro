  ...
  return (
    <div className="route-enter">
      ...
      <section className="overflow-hidden rounded-xl border border-border bg-card/75">
        ...
        {carregando ? <div className="space-y-3 p-5"><div className="skeleton h-12 rounded-lg" /><div className="skeleton h-12 rounded-lg" /><div className="skeleton h-12 rounded-lg" /></div> : (Array.isArray(dados?.solicitacoes) && dados.solicitacoes.length > 0) ? <>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[730px] text-left"><thead><tr className="border-b border-border text-[9px] font-bold uppercase tracking-[.11em] text-muted-foreground"><th className="px-4 py-3">Cotista</th><th className="px-4 py-3">Número do voo</th><th className="px-4 py-3">Rota</th><th className="px-4 py-3">Data</th><th className="px-4 py-3">Aeronave</th><th className="px-4 py-3">Passageiros</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Ação</th></tr></thead><tbody>{dados.solicitacoes.slice(0, 12).map((item) => <LinhaVoo key={item.id} item={item} onOpen={() => aoNavegar("agendamentos")} />)}</tbody></table></div>
          <div className="space-y-2 p-3 md:hidden">{dados.solicitacoes.slice(0, 12).map((item) => <CartaoVoo key={item.id} item={item} onOpen={() => aoNavegar("agendamentos")} />)}</div>
        </> : <EstadoVazio label="Nenhuma solicitação futura encontrada" />}
      </section>
...
function LinhaVoo({ item, onOpen }: { item: SolicitacaoVooInterna; onOpen: () => void }) {
  return <tr className="border-b border-border/60 last:border-0 hover:bg-secondary/20"><td className="px-4 py-3"><p className="max-w-[180px] truncate text-[10px] font-bold">{item.socio_nome || item.cliente_razao_social || item.codigo_cliente || "Cotista não informado"}</p><p className="mt-1 font-mono text-[9px] text-muted-foreground">{item.codigo_cliente || item.id.slice(0, 8)}</p></td><td className="px-4 py-3 font-mono text-[10px] font-bold text-primary">{item.numero_voo || "A definir"}</td><td className="px-4 py-3 font-mono text-[10px] text-muted-foreground">{item.origem} → {item.destino}</td><td className="px-4 py-3"><p className="text-[10px] font-semibold">{formatarData(item.data_agendada)}</p><p className="mt-1 font-mono text-[9px] text-muted-foreground">{item.horario_previsto_agendamento || "A definir"}</p></td><td className="px-4 py-3 font-mono text-[10px] font-bold">{item.matricula_registro || "A definir"}</td><td className="px-4 py-3 font-mono text-[10px] text-muted-foreground">{item.numero_passageiros}</td><td className="px-4 py-3"><EtiquetaStatus tone={tomStatus(item.status)}>{statusLabel(item.status)}</EtiquetaStatus></td><td className="px-4 py-3 text-right"><button type="button" onClick={onOpen} className="text-[10px] font-bold text-primary hover:underline">Abrir</button></td></tr>;
}

function CartaoVoo({ item, onOpen }: { item: SolicitacaoVooInterna; onOpen: () => void }) {
  return <article className="rounded-xl border border-border/70 bg-background/30 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-bold">{item.socio_nome || item.cliente_razao_social || item.codigo_cliente || "Cotista não informado"}</p><p className="mt-1 font-mono text-[9px] text-muted-foreground">{item.codigo_cliente || item.id.slice(0, 8)}</p></div><EtiquetaStatus tone={tomStatus(item.status)}>{statusLabel(item.status)}</EtiquetaStatus></div><div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 text-[10px]"><div><span className="block text-[9px] text-muted-foreground">Rota</span><strong className="font-mono">{item.origem} → {item.destino}</strong></div><div><span className="block text-[9px] text-muted-foreground">Data</span><strong>{formatarData(item.data_agendada)}</strong></div><div><span className="block text-[9px] text-muted-foreground">Voo / horário</span><strong className="font-mono text-primary">{item.numero_voo || "A definir"} · {item.horario_previsto_agendamento || "—"}</strong></div><div><span className="block text-[9px] text-muted-foreground">Aeronave / passageiros</span><strong className="font-mono">{item.matricula_registro || "A definir"} · {item.numero_passageiros ?? 0}</strong></div></div><button type="button" onClick={onOpen} className="mt-3 w-full rounded-lg border border-primary/30 px-3 py-2 text-[10px] font-bold text-primary hover:bg-primary/10">Abrir agendamento</button></article>;
}
