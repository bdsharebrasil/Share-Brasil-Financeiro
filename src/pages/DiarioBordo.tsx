  ...
  if (selected) {
    ...
    return (
      <div className="diario-bordo route-enter min-w-0 max-w-full space-y-4 md:space-y-6">
        <section className={`${card} relative overflow-hidden p-3 sm:p-4 md:p-5`}>
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between md:gap-4">
            <div className="flex min-w-0 items-center gap-2.5">
              <button type="button" onClick={backToList} aria-label="Voltar para os diários" className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border bg-secondary/50 text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground"><ArrowLeft size={14} /></button>
              <div className="min-w-0">
                <p className="text-[9px] font-bold uppercase tracking-[.16em] text-primary">Diário de bordo</p>
                <p className="mt-0.5 truncate text-sm font-extrabold text-foreground">{selected.matricula_registro}</p>
              </div>
            </div>
            ...
            <div className="flex min-w-0 w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
              <div className="flex w-full flex-col gap-2 sm:order-1 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end">
                {!closed && month && <Button type="button" onClick={() => setEntryEditor(null)} aria-label="Novo lançamento de voo" className="h-10 w-full justify-center gap-1.5 overflow-hidden rounded-[7px] border border-[rgba(195,64,4,0.35)] bg-[rgba(255,159,50,0.81)] text-[10px] font-extrabold text-orange-950 [text-shadow:1px_1px_0_rgba(0,0,0,1)] hover:bg-[rgba(255,159,50,0.9)] hover:text-orange-950 sm:h-9 sm:w-auto"><Plus size={13} strokeWidth={2.5} /> Novo lançamento</Button>}
...
function LancamentosTableConfigurada({ lancamentos, closed, onEdit, onDelete }: { lancamentos: DiarioLancamento[]; closed: boolean; onEdit: (entry: DiarioLancamento) => void; onDelete: (entry: DiarioLancamento) => void }) {
  ...
  const colunas = [
    ...
    { key: "de", label: "DE", width: 54, render: (entry: DiarioLancamento) => entry.aerodromo_partida },
    { key: "para", label: "PARA", width: 54, render: (entry: DiarioLancamento) => entry.aerodromo_chegada },
    { key: "ac", label: "AC", width: 50, render: (entry: DiarioLancamento) => time(entry.tempo_ac) },
    { key: "dep", label: "DEP", width: 50, render: (entry: DiarioLancamento) => time(entry.tempo_dep) },
    { key: "pou", label: "POU", width: 50, render: (entry: DiarioLancamento) => time(entry.tempo_pou) },
    { key: "cor", label: "COR", width: 50, render: (entry: DiarioLancamento) => time(entry.tempo_cor) },
    { key: "tvoo", label: "T VOO", width: 58, className: "text-primary", render: (entry: DiarioLancamento) => decimal(entry.tempo_voo, 2) },
    { key: "tdia", label: "T DIA", width: 58, className: "text-primary", render: (entry: DiarioLancamento) => decimal(entry.horas_diurnas, 2) },
    { key: "tnoit", label: "T NOIT", width: 60, className: "text-violet-400", render: (entry: DiarioLancamento) => decimal(entry.horas_noturnas, 2) },
    { key: "ifr", label: "IFR", width: 50, className: "text-amber-400", render: (entry: DiarioLancamento) => decimal(entry.tempo_ifr, 2) },
    { key: "pousos", label: "POUSOS", width: 58, className: "text-center text-emerald-400", render: (entry: DiarioLancamento) => entry.pousos_total },
    { key: "abast", label: "ABAST+", width: 58, className: "text-amber-400", render: (entry: DiarioLancamento) => decimal(entry.litros_combustivel_abastecido, 1) },
    { key: "fuel", label: "FUEL", width: 54, className: "text-amber-400", render: (entry: DiarioLancamento) => decimal(entry.litros_combustivel_inicio_voo, 1) },
    { key: "celula", label: "CÉLULA", width: 68, render: (entry: DiarioLancamento) => `${decimal(entry.celula, 1)}h` },
    { key: "pic", label: "PIC", width: 100, render: (entry: DiarioLancamento) => entry.pic_nome_exibicao || entry.pic_nome || entry.pic_canac || "—" },
    { key: "sic", label: "SIC", width: 100, render: (entry: DiarioLancamento) => entry.sic_nome_exibicao || entry.sic_nome || entry.sic_canac || "—" },
    { key: "voopara", label: "VOO PARA", width: 120, render: (entry: DiarioLancamento) => nomeDestino(entry) },
  ];
  ...
  const [largurasColunas, setLargurasColunas] = useState<Record<string, number>>(() => Object.fromEntries(colunas.map((coluna) => [coluna.key, coluna.width])));
  const iniciarRedimensionamento = (event: React.PointerEvent<HTMLSpanElement>, key: string, larguraAtual: number) => {
    event.preventDefault();
    event.stopPropagation();
    const inicioX = event.clientX;
    const mover = (moveEvent: PointerEvent) => setLargurasColunas((current) => ({ ...current, [key]: Math.max(44, larguraAtual + moveEvent.clientX - inicioX) }));
  ...
  return <section className={`${card} overflow-hidden`}>
    ...
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-b border-border bg-muted/30 px-4 py-3 text-xs text-muted-foreground"><div className="flex min-w-0 items-center gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-border text-[11px] font-bold">!</span><span>Clique no <strong className="font-semibold text-primary">código de referência</strong> para editar ou excluir.</span></div><span className="hidden h-4 w-px bg-border sm:block" /><div className="flex min-w-0 items-center gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-border text-[11px] font-bold">↔</span><span>Arraste a tabela horizontalmente para consultar todas as colunas. Use a divisória do cabeçalho para aumentar ou diminuir uma coluna.</span></div></div>
    ...
    {!exibidos.length ? <EstadoVazio label={aba === "emprestimos" ? "Nenhum voo empréstimo neste mês" : "Nenhum voo registrado neste mês"} /> : aba === "resumo" ? <div className="w-full max-w-full overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[720px] table-fixed border-collapse text-left text-[10px] lg:min-w-0"><thead className="border-b border-border bg-muted/40"><tr className="text-[11px] font-bold uppercase tracking-[.08em] text-muted-foreground"><th className="px-4 py-3" /><th className="px-4 py-3">DATA</th><th className="px-4 py-3">TRECHO</th><th className="px-4 py-3">DEP</th><th className="px-4 py-3">POU</th><th className="px-4 py-3 text-primary">T VOO</th><th className="px-4 py-3">VOO PARA</th></tr></thead><tbody>{exibidos.map((entry, index) => <tr key={entry.id} className={`border-b border-border/60 ${index % 2 === 0 ? "bg-card/40" : "bg-card/10"} hover:bg-primary/5`}><td className="px-4 py-3"><button type="button" onClick={() => onEdit(entry)} className="font-mono font-bold text-primary underline decoration-primary/50 underline-offset-2 hover:text-primary/80">#{entry.numero_sequencial}</button></td><td className="whitespace-nowrap px-4 py-3 text-foreground">{date(entry.data_registro)}</td><td className="px-4 py-3 font-mono font-bold text-foreground">{entry.trecho || `${entry.aerodromo_partida} X ${entry.aerodromo_chegada}`}</td><td className="px-4 py-3 font-mono text-muted-foreground">{time(entry.tempo_dep)}</td><td className="px-4 py-3 font-mono text-muted-foreground">{time(entry.tempo_pou)}</td><td className="px-4 py-3 font-mono font-bold text-primary">{decimal(entry.tempo_voo, 2)}</td><td className="max-w-[260px] truncate px-4 py-3 font-bold text-foreground">{nomeDestino(entry)}</td></tr>)}</tbody></table></div> : <div ref={scrollRef} className="diario-table-scroll w-full max-w-full cursor-grab select-none overflow-x-auto overscroll-x-contain active:cursor-grabbing" onPointerDown={iniciarArraste} onPointerMove={moverArraste} onPointerUp={pararArraste} onPointerCancel={pararArraste} onClickCapture={bloquearCliqueAposArraste}><table className="w-full min-w-[1080px] border-collapse text-left text-xs lg:min-w-0" style={{ tableLayout: "fixed" }}><colgroup>{colunas.map((coluna) => <col key={coluna.key} style={{ width: largurasColunas[coluna.key] ?? coluna.width }} />)}</colgroup><thead className="sticky top-0 z-10 border-b border-border bg-muted/80 backdrop-blur"><tr className="text-[11px] font-bold uppercase tracking-[.08em] text-muted-foreground">{colunas.map((coluna) => <th key={coluna.key} className={`relative whitespace-nowrap text-ellipsis border-r border-border px-1.5 py-2 text-[8px] select-none sm:px-2 sm:py-3 sm:text-[9px] ${coluna.className || ""}`} style={{ width: largurasColunas[coluna.key] ?? coluna.width }}><span className="block truncate pr-2">{coluna.label}</span><span role="separator" aria-label={`Redimensionar coluna ${coluna.label || "de referência"}`} onPointerDown={(event) => iniciarRedimensionamento(event, coluna.key, largurasColunas[coluna.key] ?? coluna.width)} className="absolute inset-y-0 right-0 z-10 w-1.5 cursor-col-resize rounded-sm bg-transparent transition-colors hover:bg-primary/70 active:bg-primary" /></th>)}</tr></thead><tbody className="text-xs text-foreground">{exibidos.map((entry, index) => <tr key={entry.id} className={`border-b border-border/50 ${index % 2 === 0 ? "bg-card/40" : "bg-card/10"} hover:bg-primary/5`}><td className="relative border-r border-border px-2 py-2 text-center font-mono font-bold text-primary sm:px-3 sm:py-3"><button type="button" onClick={() => setAcoesAberta(acoesAberta === entry.id ? null : entry.id)} className="underline decoration-primary/50 underline-offset-2 hover:text-primary/80">#{entry.numero_sequencial}</button>{acoesAberta === entry.id && <div className="absolute left-2 top-10 z-20 flex w-36 flex-col gap-1 rounded-xl border border-border bg-card p-1.5 text-left shadow-xl"><button type="button" onClick={() => { setAcoesAberta(null); onEdit(entry); }} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-foreground hover:bg-primary/10 hover:text-primary"><Edit3 size={13} /> Editar</button><button type="button" onClick={() => { setAcoesAberta(null); onDelete(entry); }} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 size={13} /> Excluir</button></div>}</td>{colunas.slice(1).map((coluna) => <td key={coluna.key} className={`whitespace-nowrap overflow-hidden text-ellipsis border-r border-border/60 px-1.5 py-2 font-mono text-[8px] sm:px-2 sm:py-3 sm:text-[9px] ${coluna.className || ""}`}>{coluna.render(entry)}</td>)}</tr>)}</tbody><tfoot className="sticky bottom-0 border-t-2 border-primary/30 bg-card text-xs font-bold"><tr><td className="px-2 py-2 text-center text-primary sm:px-3 sm:py-3">TOTAL</td><td colSpan={7} /><td className="px-3 py-3 font-mono text-primary">{decimal(totais.voo, 2)}</td><td colSpan={3} /><td className="px-3 py-3 text-center text-emerald-400">{totais.pousos}</td><td className="px-3 py-3 text-amber-400">{decimal(totais.abast, 1)}</td><td className="px-3 py-3 text-amber-400">{decimal(totais.fuel, 1)}</td><td colSpan={4} /></tr></tfoot></table></div>}
  </section>;
}
  ...
  return (
    <section className="task-compose relative overflow-hidden rounded-2xl border p-5 shadow-2xl">
      ...
      <div className="relative space-y-6">
        ...
        {temDiaria && <FormSection icon={<BookOpen size={13} />} title="Diárias"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><NumberField label="Quantidade de diárias" value={value("diarias")} onChange={(v) => setField("diarias", v)} step="1" /></div></FormSection>}
