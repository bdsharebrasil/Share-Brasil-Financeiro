...
type TipoEmissao = "recibo_reembolso" | "recibo_colaborador" | "recibo_pagamento";
type Formulario = {
...
const opcoesTipo: Array<{ id: TipoEmissao; titulo: string; detalhe: string; icon: typeof Receipt; cor: string; fundo: string }> = [
  { id: "recibo_reembolso", titulo: "Recibo de reembolso", detalhe: "Despesa antecipada pela Share para um cotista.", icon: RotateCcw, cor: "text-amber-500", fundo: "bg-amber-500/[.06]" },
  { id: "recibo_colaborador", titulo: "Recibo colaborador", detalhe: "Despesa da Share vinculada ao colaborador.", icon: Receipt, cor: "text-primary", fundo: "bg-primary/[.06]" },
  { id: "recibo_pagamento", titulo: "Recibo de pagamento", detalhe: "Lançamento simples, sem vínculo com cotista ou colaborador.", icon: FileText, cor: "text-sky-500", fundo: "bg-sky-500/[.06]" },
];
...
export default function EmissaoRecibo({ aoVoltar }: { aoVoltar: () => void }) {
  ...
  const recebedorItems = useMemo(() => opcoes.recebedores.map((recebedor) => ({
    id: recebedor.id,
    label: [recebedor.nome || recebedor.nome_completo, recebedor.cpf && `CPF: ${recebedor.cpf}`, recebedor.endereco, [recebedor.cidade, recebedor.uf].filter(Boolean).join("/")].filter(Boolean).join(" · "),
  })), [opcoes.recebedores]);
  const mostrarMetadadosPagamento = form.tipo === "recibo_pagamento" && form.pagador_tipo === "cotista_aeronave";
  ...
  const categoriaValida = form.tipo === "recibo_reembolso"
    ? Boolean(form.categoria_id && form.categoria_nome && /REEMBOLS/i.test(form.categoria_nome))
  ...
  const emitir = async () => {
    ...
    try {
      const payload: CriarReciboPayload = {
        ...
        rateado: (form.tipo === "recibo_reembolso" || form.tipo === "recibo_pagamento") && form.rateado,
        aeronave_id: form.rateado ? form.aeronave_id || null : form.tipo === "recibo_pagamento" ? null : form.aeronave_id || null,
        cliente_id: form.tipo === "recibo_reembolso" ? form.cliente_id || null : null,
        colaborador_id: form.tipo === "recibo_colaborador" ? form.colaborador_id : null,
        recebedor_id: form.tipo === "recibo_pagamento" ? form.recebedor_id || null : null,
        recebedor_nome: form.tipo === "recibo_pagamento" ? form.recebedor_nome.trim() : null,
        recebedor_cpf: form.tipo === "recibo_pagamento" ? form.recebedor_cpf.trim() || null : null,
        recebedor_endereco: form.tipo === "recibo_pagamento" ? form.recebedor_endereco.trim() || null : null,
        recebedor_cidade: form.tipo === "recibo_pagamento" ? form.recebedor_cidade.trim() || null : null,
        recebedor_uf: form.tipo === "recibo_pagamento" ? form.recebedor_uf.trim() || null : null,
  ...
  return (
    <div className="route-enter mx-auto max-w-6xl space-y-5">
      ...
      {abaAtiva === "emissao" && <section className="overflow-hidden rounded-sm border border-border bg-card/60 shadow-lg">
        ...
        {form.tipo && <div className="border-t border-border px-5 py-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {form.tipo === "recibo_pagamento" ? <><Campo label="RECEBEDOR" obrigatorio><SearchableCombobox items={recebedorItems} value={form.recebedor_id} onChange={(id, label) => { const recebedor = opcoes.recebedores.find((item) => item.id === id); alterar("recebedor_id", id); alterar("recebedor_nome", recebedor?.nome || recebedor?.nome_completo || label); alterar("recebedor_cpf", recebedor?.cpf || ""); alterar("recebedor_endereco", recebedor?.endereco || ""); alterar("recebedor_cidade", recebedor?.cidade || ""); alterar("recebedor_uf", recebedor?.uf || ""); }} placeholder="Selecione o recebedor" searchPlaceholder="Buscar por nome, CPF, e-mail..." emptyMessage="Nenhum recebedor encontrado." allowFreeText /></Campo><Campo label="CPF do recebedor"><input value={form.recebedor_cpf} onChange={(e) => alterar("recebedor_cpf", e.target.value)} placeholder="000.000.000-00" inputMode="numeric" className="campo" /></Campo></> : form.tipo !== "recibo_colaborador" ? <Campo label="Cliente" obrigatorio><select value={form.cliente_id} onChange={(e) => alterar("cliente_id", e.target.value)} className="campo"><option value="">Selecione o cliente</option>{opcoes.clientes.map((cliente) => <option key={cliente.id} value={cliente.id}>{cliente.razao_social}</option>)}</select></Campo> : <Campo label="RECEBEDOR" obrigatorio><select value={form.colaborador_id} onChange={(e) => alterar("colaborador_id", e.target.value)} className="campo"><option value="">Selecione o colaborador</option>{opcoes.colaboradores.map((colaborador) => <option key={colaborador.id} value={colaborador.id}>{colaborador.nome_exibicao || colaborador.nome_completo}</option>)}</select></Campo>}
            ...
            <Campo label="Número do recibo"><input value="Gerado automaticamente" readOnly aria-readonly="true" className="campo font-mono text-muted-foreground" /></Campo>
            {form.tipo === "recibo_pagamento" && <><Campo label="Pagador" obrigatorio><SearchableCombobox items={[{ id: "empresa", label: "Share Brasil" }, ...opcoes.cotistas.map((cotista) => ({ id: cotista.id, label: `${cotista.nome}${cotista.codigo_cliente ? ` · ${cotista.codigo_cliente}` : ""}` }))]} value={form.pagador_tipo === "empresa" ? "empresa" : form.pagador_id} onChange={(id) => { if (id === "empresa") { alterar("pagador_tipo", "empresa"); alterar("pagador_id", ""); } else { const cotista = opcoes.cotistas.find((item) => item.id === id); alterar("pagador_tipo", "cotista_aeronave"); alterar("pagador_id", id); alterar("categoria_id", ""); if (cotista) alterar("recebedor_nome", form.recebedor_nome); } }} placeholder="Selecione o pagador" searchPlaceholder="Buscar cotista..." emptyMessage="Nenhum cotista encontrado." /></Campo><div className="rounded-sm border border-primary/30 bg-primary/[.06] p-3"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-primary">DADOS DO PAGADOR</p><p className="mt-1 text-[11px] font-semibold">{form.pagador_tipo === "cotista_aeronave" ? `${pagadorSelecionado?.nome || "Cotista"} · ${pagadorSelecionado?.cnpj || pagadorSelecionado?.cpf || "Documento não informado"}` : "Share Brasil"}</p></div></>}
          ...
          {form.tipo !== "recibo_colaborador" && form.tipo !== "recibo_pagamento" && <div className="mt-5 rounded-sm border border-border bg-secondary/[.12] p-4">
      ...
      {abaAtiva === "emissao" && previewAberta && form.tipo && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"><div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-sm bg-white p-6 text-slate-800 shadow-2xl"><div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4"><img src={logoShare} alt="Share Brasil" className="h-20 w-40 object-contain object-center p-1" /><div className="text-center"><h2 className="text-2xl font-black tracking-wide underline">RECIBO</h2><p className="mt-1 text-[10px] text-slate-500">Pré-visualização antes da finalização</p></div><div className="text-right"><p className="text-[10px] font-bold">Número do recibo</p><p className="font-mono text-[11px] font-bold text-slate-500">{form.numero_recibo.trim() || "Será gerado ao finalizar"}</p><p className="mt-2 border-2 border-slate-800 px-4 py-2 text-lg font-black">{moeda(valorNumerico(form.valor))}</p></div></div><div className="grid gap-6 border-b border-slate-300 py-6 text-xs md:grid-cols-2">{form.tipo === "recibo_pagamento" && form.pagador_tipo === "cotista_aeronave" ? <><div><p className="mb-1 text-[9px] font-bold uppercase text-slate-500">Recebedor</p><strong>{form.recebedor_nome || "Recebedor não informado"}</strong><p>CPF: {form.recebedor_cpf || "Não informado"}</p><p>{form.recebedor_endereco}</p><p>{[form.recebedor_cidade, form.recebedor_uf].filter(Boolean).join(" - ")}</p></div><div><p className="mb-1 text-[9px] font-bold uppercase text-slate-500">Pagador</p><strong>{pagadorSelecionado?.nome || "Cotista"}</strong><p>{pagadorSelecionado?.cnpj || pagadorSelecionado?.cpf || "Documento não informado"}</p><p>{pagadorSelecionado?.endereco || "Endereço não informado"}</p><p>{[pagadorSelecionado?.cidade, pagadorSelecionado?.uf].filter(Boolean).join(" - ")}</p></div></> : <><div><p className="mb-1 text-[9px] font-bold uppercase text-slate-500">{form.tipo === "recibo_colaborador" ? "Pagador" : "Emissor"}</p><strong>SHARE BRASIL SERVIÇOS AERONÁUTICOS</strong><p>CNPJ: 30.898.549/0001-06</p><p>Av. Presidente Arthur Bernardes, 1457</p><p>Várzea Grande - 78125-100</p></div><div><p className="mb-1 text-[9px] font-bold uppercase text-slate-500">{form.tipo === "recibo_colaborador" ? "Recebedor" : form.tipo === "recibo_pagamento" ? "Recebedor" : "Pagador"}</p><strong>{form.tipo === "recibo_colaborador" ? (colaboradorSelecionado?.nome_completo || "Colaborador") : form.tipo === "recibo_pagamento" ? (form.recebedor_nome || "Recebedor") : (pagadorSelecionado?.nome || "SHARE BRASIL")}</strong><p>{form.tipo === "recibo_colaborador" ? `CPF: ${colaboradorSelecionado?.cpf || "não informado"}` : form.tipo === "recibo_pagamento" ? `CPF: ${form.recebedor_cpf || "não informado"}` : (pagadorSelecionado?.cnpj || pagadorSelecionado?.cpf || "CNPJ: 30.898.549/0001-06")}</p><p>{form.tipo === "recibo_colaborador" ? "" : form.tipo === "recibo_pagamento" ? form.recebedor_endereco : (pagadorSelecionado?.endereco || "Av. Presidente Arthur Bernardes, 1457")}</p><p>{form.tipo === "recibo_pagamento" ? [form.recebedor_cidade, form.recebedor_uf].filter(Boolean).join(" - ") : form.tipo === "recibo_colaborador" ? "" : ([pagadorSelecionado?.cidade, pagadorSelecionado?.uf].filter(Boolean).join(" - ") || "Várzea Grande - MT") }</p></div></>}</div>{(() => { const temDoc = Boolean(form.numero_documento_anexo); const cols = temDoc ? "grid-cols-[1fr_130px_100px]" : "grid-cols-[1fr_100px]"; return <div className="overflow-hidden border border-slate-300 text-xs"><div className={`grid ${cols} bg-slate-200 p-2 font-bold`}><span>Descrição do Serviço</span>{temDoc && <span>Nº Documento</span>}<span>Valor</span></div><div className={`grid ${cols} p-3`}><span>{form.descricao_servico}</span>{temDoc && <span>{form.numero_documento_anexo}</span>}<strong>{moeda(valorNumerico(form.valor))}</strong></div></div>; })()}<>{form.tipo !== "recibo_pagamento" && form.tipo !== "recibo_colaborador" && <div className="mt-5 border border-slate-200 p-3 text-xs"><p className="mb-2 text-[9px] font-bold uppercase text-slate-500">Dados do recibo</p><p>Categoria: {categoriaClienteSelecionada?.nome || form.categoria_nome || "—"}</p>{mostrarMetadadosPagamento && <><p>Periodicidade: {form.periodicidade} · Tipo de rateio: {form.tipo_rateio}</p>{[form.subcategoria_1, form.subcategoria_2, form.subcategoria_3, form.subcategoria_4].filter(Boolean).length > 0 && <p>{[form.subcategoria_1, form.subcategoria_2, form.subcategoria_3, form.subcategoria_4].filter(Boolean).join(" · ")}</p>}</>}</div>}</><>{form.observacoes && <div className="mt-5 border border-slate-200 p-3 text-xs"><p className="mb-2 text-[9px] font-bold uppercase text-slate-500">Observações</p><p className="whitespace-pre-wrap">{form.observacoes}</p></div>}<div className="mt-5 border-t border-slate-200 pt-4 text-center"><img src={assinaturaRecibo} alt="Assinatura" className="mx-auto h-12 w-auto" /><p className="text-xs font-semibold">Rolffe de Lima Erbe</p><p className="text-[10px] text-slate-500">Gestor Responsável</p></div></><div className="mt-5 flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setPreviewAberta(false)} className="h-9 text-xs">Voltar e editar</Button><Button type="button" onClick={emitir} disabled={salvando} className="h-9 text-xs">{salvando ? "Salvando..." : "Confirmar e finalizar recibo"}</Button></div></div></div>}
