...
app.get('/api/interno/agendamento', async c => {
  ...
  const [agendamentos, aeronave, tripulacao, freelancers, disponibilidades] = await Promise.all([
    db.prepare(`SELECT s.id, s.cliente_id, s.socio_id, s.cliente_emprestimo_id, s.socio_emprestimo_id, s.aeronave_id, s.origem, s.destino, s.data_agendada, date(s.data_agendada, '+' || (COALESCE(s.dias_duracao, 1) - 1) || ' days') AS data_fim, s.horario_previsto_agendamento, s.dias_duracao, s.numero_passageiros, s.voo_emprestado, s.status, s.observacoes, s.motivo_rejeicao, s.numero_voo, s.criado_em, s.atualizado_em, s.piloto_id, COALESCE((SELECT nome_completo FROM tripulacao WHERE id = s.piloto_id LIMIT 1), (SELECT nome_completo FROM tripulacao_freelancer WHERE id = s.piloto_id LIMIT 1)) AS piloto_nome, s.copiloto_id, COALESCE((SELECT nome_completo FROM tripulacao WHERE id = s.copiloto_id LIMIT 1), (SELECT nome_completo FROM tripulacao_freelancer WHERE id = s.copiloto_id LIMIT 1)) AS copiloto_nome, c.razao_social AS cliente_razao_social, so.nome AS socio_nome, ce.razao_social AS cliente_emprestimo_nome, se.nome AS socio_emprestimo_nome, COALESCE(ce.codigo_cliente, cae.codigo_cliente, c.codigo_cliente, ca.codigo_cliente) AS codigo_cliente, a.matricula_registro, a.modelo, a.status AS status_aeronave, (SELECT cp.status FROM checklists_pre_voo cp WHERE cp.solicitacao_id = s.id ORDER BY cp.criado_em DESC LIMIT 1) AS checklist_status
      ...
      LEFT JOIN cotista_aeronave ca ON ca.id = (SELECT ca2.id FROM cotista_aeronave ca2 WHERE (ca2.cliente_id = s.cliente_id OR ca2.socio_id = s.socio_id) AND ca2.aeronave_id = s.aeronave_id ORDER BY ca2.id LIMIT 1)
      LEFT JOIN cotista_aeronave cae ON cae.id = (SELECT cae2.id FROM cotista_aeronave cae2 WHERE (cae2.cliente_id = s.cliente_emprestimo_id OR cae2.socio_id = s.socio_emprestimo_id) AND cae2.aeronave_id = s.aeronave_id ORDER BY cae2.id LIMIT 1)
      LEFT JOIN aeronave a ON a.id = s.aeronave_id
...
app.get('/api/interno/agendamento/:id/checklist', async c => {
  ...
  const row = await c.env.SHARE_DB.prepare('SELECT id, solicitacao_id, respostas AS itens, observacoes, abastecimento_id, precisa_abastecer, status, executado_por AS usuario_id, executado_por_nome, nivel_oleo, alerta_id, concluido_em FROM checklists_pre_voo WHERE solicitacao_id = ? ORDER BY criado_em DESC LIMIT 1').bind(c.req.param('id')).first<any>()
